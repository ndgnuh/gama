...
public class Polygon extends ShapeElement {
	 ...
	 */
	/*
	 * public void build() { StyleAttribute styleAttrib = getStyle("fill-rule"); String fillRule = (styleAttrib == null)
	 * ? "nonzero" : styleAttrib.getStringValue();
	 *
	 * path = new GeneralPath( fillRule.equals("evenodd") ? GeneralPath.WIND_EVEN_ODD : GeneralPath.WIND_NON_ZERO,
	 * points.length / 2);
	 *
	 * path.moveTo(points[0], points[1]); for (int i = 2; i < points.length; i += 2) { path.lineTo(points[i], points[i +
	 * 1]); } path.closePath(); }
	 */
	...
	// final int newVal =
	// sty.getStringValue().equals("evenodd") ? GeneralPath.WIND_EVEN_ODD : GeneralPath.WIND_NON_ZERO;
	// if (newVal != fillRule) {
	// fillRule = newVal;
	// shapeChange = true;
	// }
	...
	//
	// if (getPres(sty.setName("points"))) {
	// final String newVal = sty.getStringValue();
