/*********************************************************************************************
 *
 * 'GamlExpressionCompiler.java, in plugin gama.core.lang, is part of the source code of the GAMA modeling and
...
import static gama.common.interfaces.IKeyword.AS;
...
import static gama.common.interfaces.IKeyword.SELF;
import static gama.common.interfaces.IKeyword.SPECIES;
import static gama.common.interfaces.IKeyword.SUPER;
import static gama.common.interfaces.IKeyword.TRUE;
import static gama.common.interfaces.IKeyword.UNKNOWN;
import static gama.common.interfaces.IKeyword._DOT;
import static gaml.expressions.IExpressionFactory.FALSE_EXPR;
...
import gaml.expressions.EachExpression;
...
public class GamlExpressionCompiler extends GamlSwitch<IExpression> implements IExpressionCompiler<Expression> {
	...
	private IExpression binary(final String op, final IExpression left, final Expression originalExpression) {
		...
		// casting type
		if (AS.equals(op))
			return binaryAs(left, rightMember);
		// if the operator is "is", the right-hand expression should be a type
		if (IS.equals(op))
			return binaryIs(left, rightMember);
	...
	public IExpression caseDot(final Access object) {
		...
		if (right instanceof StringLiteral)
			...
			return compileFieldExpr(object.getLeft(), right);
		return null;
	}
	...
	private IExpression tryCastingFunction(final String op, final Function object) {
		if (!isCastingFunction(op, object))
			return null;
		final List<Expression> args = EGaml.getInstance().getExprsOf(object.getRight());
