/*********************************************************************************************
 *
 * 'WebHelper.java, in plugin gama.ui.base.shared, is part of the source code of the GAMA modeling and simulation
...
public class WebHelper {
	...
	public static void openWelcomePage(final boolean ifEmpty) {
		...
		try {
			WorkbenchHelper.getPage().openEditor(input, "gama.core.application.browser");
	...
	public static void openPage(final String string) {
		try {
			final IGamaView.Html view =
					(Html) WorkbenchHelper.getPage().openEditor(new NullEditorInput(), "gama.core.application.browser");
			view.setUrl(string);
		} catch (final PartInitException e) {
			e.printStackTrace();
		}
