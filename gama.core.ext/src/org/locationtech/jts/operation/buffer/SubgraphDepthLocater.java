...
/**
 * Locates a subgraph inside a set of subgraphs, in order to determine the outside depth of the subgraph. The input
 * subgraphs are assumed to have had depths already calculated for their edges.
 *
 * @version 1.7
 */
class SubgraphDepthLocater {
	...
	public int getDepth(final Coordinate p) {
		final List stabbedSegments = findStabbedSegments(p);
		// if no segments on stabbing line subgraph must be outside all others.
		if (stabbedSegments.size() == 0) { return 0; }
		final DepthSegment ds = (DepthSegment) Collections.min(stabbedSegments);
		return ds.leftDepth;
	}

	/**
	 * Finds all non-horizontal segments intersecting the stabbing line. The stabbing line is the ray to the right of
	 * stabbingRayLeftPt.
	 *
	 ...
	 */
	private List findStabbedSegments(final Coordinate stabbingRayLeftPt) {
	...
	/**
	 * Finds all non-horizontal segments intersecting the stabbing line in the list of dirEdges. The stabbing line is
	 * the ray to the right of stabbingRayLeftPt.
	 *
	 * @param stabbingRayLeftPt
	 *            the left-hand origin of the stabbing line
	 ...
	 */
	private void findStabbedSegments(final Coordinate stabbingRayLeftPt, final List dirEdges,
			final List<DepthSegment> stabbedSegments) {
		/**
		 * Check all forward DirectedEdges only. This is still general, because each Edge has a forward DirectedEdge.
		 */
	...
	/**
	 * Finds all non-horizontal segments intersecting the stabbing line in the input dirEdge. The stabbing line is the
	 * ray to the right of stabbingRayLeftPt.
	 *
	 * @param stabbingRayLeftPt
	 *            the left-hand origin of the stabbing line
	 * @param stabbedSegments
	 *            the current list of {@link DepthSegments} intersecting the stabbing line
	 */
	private void findStabbedSegments(final Coordinate stabbingRayLeftPt, final DirectedEdge dirEdge,
			final List<DepthSegment> stabbedSegments) {
		final Coordinate[] pts = dirEdge.getEdge().getCoordinates();
		for (int i = 0; i < pts.length - 1; i++) {
			...
			final double maxx = Math.max(seg.p0.x, seg.p1.x);
	...
	static class DepthSegment implements Comparable {
		...
		/**
		 * Defines a comparison operation on DepthSegments which orders them left to right. Assumes the segments are
		 * normalized.
		 * <p>
		 ...
		 * KNOWN BUGS:
		 * <ul>
		 * <li>The logic does not obey the {@link Comparator.compareTo} contract. This is acceptable for the intended
		 * usage, but may cause problems if used with some utilities in the Java standard library (e.g.
		...
		public int compareTo(final Object obj) {
			...
			/**
			 * try and compute a determinate orientation for the segments. Test returns 1 if other is left of this (i.e.
			 * this > other)
			 */
			...
			/**
			 * If comparison between this and other is indeterminate, try the opposite call order. The sign of the
		...
		/**
		 * Compare two collinear segments for left-most ordering. If segs are vertical, use vertical ordering for
		 * comparison. If segs are equal, return 0. Segments are assumed to be directed so that the second coordinate is
		 * >= to the first (e.g. up and to the right).
		 *
		 * @param seg0
		...
		private int compareX(final LineSegment seg0, final LineSegment seg1) {
			final int compare0 = seg0.p0.compareTo(seg1.p0);
