...
public class learningGraphDatabaseGenerator {
	...
	public static void main(final String[] args) throws FileNotFoundException {
		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					LearningConcept newLearningConcept =
							new LearningConcept(id, name, description, x, y, listPrerequisite);
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					for (int i = 0; i < eElement.getElementsByTagName("learningConceptAssociated").getLength(); i++) {
						associatedLearningConceptList.add(
								eElement.getElementsByTagName("learningConceptAssociated").item(i).getTextContent());
					}
					String color = eElement.getAttribute("color");
					Topic newTopic = new Topic(id, name, x, y, xBigHallow, yBigHallow, sizeBigHallow,
							associatedLearningConceptList, color);
		...
		for (int i = 0; i < learningConceptList.size(); i++) {
			...
			for (int j = 0; j < learningConceptList.get(i).m_prerequisitesList.size(); j++) {
				if (!listConceptNames.contains(learningConceptList.get(i).m_prerequisitesList.get(j))) {
					System.out.println("ERROR !!!! The name " + learningConceptList.get(i).m_prerequisitesList.get(j)
							+ " is not a learning concept id");
				}
			}
		}
		for (int i = 0; i < topicList.size(); i++) {
			...
			for (int j = 0; j < topicList.get(i).m_associatedLearningConceptList.size(); j++) {
				if (!listConceptNames.contains(topicList.get(i).m_associatedLearningConceptList.get(j))) {
					System.out.println("ERROR !!!! The name " + topicList.get(i).m_associatedLearningConceptList.get(j)
							+ " is not a learning concept id");
				}
			}
		}
		...
		for (int i = 0; i < learningConceptList.size(); i++) {
			...
			input += "{id:" + idNode + ", label:''" + ", x:" + lc.m_xPos + ", y:" + lc.m_yPos
					+ ", category:'learningConcept'},\n";
			input += "{id:" + idHallow + ", label:''" + ", x:" + lc.m_xPos + ", y:" + lc.m_yPos
					+ ", category:'highlightLearningConcept'},\n";
			input += "{id:" + idNodeWithLabel + ", label:'" + lc.m_name + "', x:" + lc.m_xPos + ", y:" + lc.m_yPos
		...
		for (int i = 0; i < topicList.size(); i++) {
			...
			input += "{id:" + idNode + ", label:'" + tp.m_name + "', x:" + tp.m_xPos + ", y:" + tp.m_yPos
					+ ", category:'topic', color:'" + tp.m_color + "'},\n";
			String colorWithSmoothAlpha = tp.m_color.split(",")[0] + "," + tp.m_color.split(",")[1] + ","
					+ tp.m_color.split(",")[2] + ",0.5)";
			input += "{id:" + idHallow + ", label:''" + ", x:" + tp.m_xPos + ", y:" + tp.m_yPos
					+ ", category:'highlightTopic', color:'" + colorWithSmoothAlpha + "'},\n";
			input += "{id:" + idBigHallow + ", label:''" + ", x:" + tp.m_xPosBigHallow + ", y:" + tp.m_yPosBigHallow
					+ ", size:" + tp.m_sizeBigHallow + ", category:'topicBigHallow', color:'" + colorWithSmoothAlpha
					+ "'},\n";
			idNb++;
		}
		...
		for (int i = 0; i < learningConceptList.size(); i++) {
			...
			for (int j = 0; j < learningConceptList.get(i).m_prerequisitesList.size(); j++) {
				int idNodePrerequisite =
						listConceptNames.indexOf(learningConceptList.get(i).m_prerequisitesList.get(j)) + 200;
		...
		for (int i = 0; i < topicList.size(); i++) {
			...
			for (int j = 0; j < topicList.get(i).m_associatedLearningConceptList.size(); j++) {
				int idLearningConcept =
						listConceptNames.indexOf(topicList.get(i).m_associatedLearningConceptList.get(j)) + 200;
