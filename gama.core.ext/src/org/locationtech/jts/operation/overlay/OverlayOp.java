...
/**
 * Computes the geometric overlay of two {@link Geometry}s. The overlay can be used to determine any boolean combination
 * of the geometries.
 *
 * @version 1.7
 */
public class OverlayOp extends GeometryGraphOperation {
	/**
	 * The spatial functions supported by this class. These operations implement various boolean combinations of the
	 * resultants of the overlay.
	 */
	...
	/**
	 * Tests whether a point with a given topological {@link Label} relative to two geometries is contained in the
	...
	/**
	 * Tests whether a point with given {@link Location}s relative to two geometries is contained in the result of
	 * overlaying the geometries using a given overlay operation.
	 * <p>
	 * The method handles arguments of {@link Location#NONE} correctly
	 *
	 * @param loc0
	...
	public OverlayOp(final Geometry g0, final Geometry g1) {
		...
		/**
		 * Use factory of primary geometry. Note that this does NOT handle mixed-precision arguments where the second
		 * arg has greater precision than the first.
		 */
	...
	private void computeOverlay(final int opCode) {
		 ...
		 *
		 * This test is slow, but necessary in order to catch robustness failure situations. If an exception is thrown
		 * because of a noding failure, then snapping will be performed, which will hopefully avoid the problem. In the
		...
		/**
		 * The ordering of building the result Geometries is important. Areas must be built before lines, which must be
		 * built before points. This is so that lines which are covered by areas are not included explicitly, and
		...
		final PointBuilder pointBuilder = new PointBuilder(this, geomFact, ptLocator);
	...
	/**
	 * Insert an edge from one of the noded input graphs. Checks edges that are inserted to see if an identical edge
	 * already exists. If so, the edge is not inserted, but its label is merged with the existing edge.
	 */
	...
	/**
	 * If either of the GeometryLocations for the existing label is exactly opposite to the one in the labelToMerge,
	 * this indicates a dimensional collapse has happened. In this case, convert the label for that Geometry to a Line
	 * label
	 */
	/*
	 * NOT NEEDED? private void checkDimensionalCollapse(Label labelToMerge, Label existingLabel) { if
	 * (existingLabel.isArea() && labelToMerge.isArea()) { for (int i = 0; i < 2; i++) { if (! labelToMerge.isNull(i) &&
	 ...
	 */
	/**
	 * Update the labels for edges according to their depths. For each edge, the depths are first normalized. Then, if
	 * the depths for the edge are equal, this edge must have collapsed into a line edge. If the depths are not equal,
	 * update the label with the locations corresponding to the depths (i.e. a depth of 0 corresponds to a Location of
	 * EXTERIOR, a depth of 1 corresponds to INTERIOR)
	 */
	private void computeLabelsFromDepths() {
		for (final Iterator it = edgeList.iterator(); it.hasNext();) {
			...
			/**
			 * Only check edges for which there were duplicates, since these are the only ones which might be the result
			 * of dimensional collapses.
			 */
			if (!depth.isNull()) {
				depth.normalize();
				for (int i = 0; i < 2; i++) {
					if (!lbl.isNull(i) && lbl.isArea() && !depth.isNull(i)) {
						/**
						 * if the depths are equal, this edge is the result of the dimensional collapse of two or more
						 * edges. It has the same location on both sides of the edge, so it has collapsed to a line.
						 */
						if (depth.getDelta(i) == 0) {
							 ...
							 * locations on both sides. The label of the edge must be updated to reflect the resultant
	...
	/**
	 * Copy all nodes from an arg geometry into this graph. The node label in the arg geometry overrides any previously
	 * computed label for that argIndex. (E.g. a node may be an intersection node with a previously computed label of
	 * BOUNDARY, but in the original arg Geometry it is actually in the interior due to the Boundary Determination Rule)
	 */
	...
	/**
	 * Compute initial labelling for all DirectedEdges at each node. In this step, DirectedEdges will acquire a complete
	 * labelling (i.e. one with labels for both Geometries) only if they are incident on a node which has edges for both
	 * Geometries
	 */
	...
	/**
	 * For nodes which have edges from only one Geometry incident on them, the previous step will have left their
	 * dirEdges with no labelling for the other Geometry. However, the sym dirEdge may have a labelling for the other
	...
	/**
	 * Incomplete nodes are nodes whose labels are incomplete. (e.g. the location for one Geometry is null). These are
	 * either isolated nodes, or nodes which have edges from only a single Geometry incident on them.
	 *
	 * Isolated nodes are found because nodes in one graph which don't intersect nodes in the other are not completely
	 * labelled by the initial process of adding nodes to the nodeList. To complete the labelling we need to check for
	 * nodes that lie in the interior of edges, and in the interior of areas.
	 * <p>
	 * When each node labelling is completed, the labelling of the incident edges is updated, to complete their
	 * labelling as well.
	 */
	private void labelIncompleteNodes() {
		...
		/*
		 * int nPoly0 = arg[0].getGeometry().getNumGeometries(); int nPoly1 = arg[1].getGeometry().getNumGeometries();
		 * System.out.println("# isolated nodes= " + nodeCount + "   # poly[0] = " + nPoly0 + "   # poly[1] = " +
		 * nPoly1);
		 */
	}
	...
	private void labelIncompleteNode(final Node n, final int targetIndex) {
		final int loc = ptLocator.locate(n.getCoordinate(), arg[targetIndex].getGeometry());
	...
	/**
	 * Find all edges whose label indicates that they are in the result area(s), according to the operation being
	 * performed. Since we want polygon shells to be oriented CW, choose dirEdges with the interior of the result on the
	 * RHS. Mark them as being in the result. Interior Area edges are the result of dimensional collapses. They do not
	 ...
	 */
	public boolean isCoveredByLA(final Coordinate coord) {
	 ...
	 */
	public boolean isCoveredByA(final Coordinate coord) {
		if (isCovered(coord, resultPolyList)) { return true; }
		return false;
	}
	 ...
	 */
	private boolean isCovered(final Coordinate coord, final List geomList) {
	...
	private Geometry computeGeometry(final List resultPointList, final List resultLineList, final List resultPolyList,
			final int opcode) {
	...
	/**
	 * Creates an empty result geometry of the appropriate dimension, based on the given overlay operation and the
	 * dimensions of the inputs. The created geometry is always an atomic geometry, not a collection.
	 * <p>
	 ...
	 * <li>{@link #SYMDIFFERENCE} - result has the dimension of the highest input dimension (since the symmetric
	 * Difference is the union of the differences).
	 * </ul>
	 *
	 * @param overlayOpCode
	 ...
	 */
	public static Geometry createEmptyResult(final int overlayOpCode, final Geometry a, final Geometry b,
