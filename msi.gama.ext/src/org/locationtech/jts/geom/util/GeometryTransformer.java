...
/**
 * A framework for processes which transform an input {@link Geometry} into an output {@link Geometry}, possibly
 * changing its structure and type(s). This class is a framework for implementing subclasses which perform
 * transformations on various different Geometry subclasses. It provides an easy way of applying specific
 * transformations to given geometry types, while allowing unhandled types to be simply copied. Also, the framework
 * ensures that if subcomponents change type the parent geometries types change appropriately to maintain valid
 * structure. Subclasses will override whichever <code>transformX</code> methods they need to to handle particular
 * Geometry types.
 * <p>
 * A typically usage would be a transformation class that transforms <tt>Polygons</tt> into <tt>Polygons</tt>,
 * <tt>LineStrings</tt> or <tt>Points</tt>, depending on the geometry of the input (For instance, a simplification
 * operation). This class would likely need to override the {@link #transformMultiPolygon(MultiPolygon, Geometry)}
 * method to ensure that if input Polygons change type the result is a <tt>GeometryCollection</tt>, not a
 * <tt>MultiPolygon</tt>.
 * <p>
 * The default behaviour of this class is simply to recursively transform each Geometry component into an identical
 * object by deep copying down to the level of, but not including, coordinates.
 * <p>
 * All <code>transformX</code> methods may return <code>null</code>, to avoid creating empty or invalid geometry
 * objects. This will be handled correctly by the transformer. <code>transform<i>XXX</i></code> methods should always
 * return valid geometry - if they cannot do this they should return <code>null</code> (for instance, it may not be
 * possible for a transformLineString implementation to return at least two points - in this case, it should return
 * <code>null</code>). The {@link #transform(Geometry)} method itself will always return a non-null Geometry object (but
 * this may be empty).
 *
 * @version 1.7
 *
...
public class GeometryTransformer {
	...
	/**
	 * <code>true</code> if a homogenous collection result from a {@link GeometryCollection} should still be a general
	 * GeometryCollection
	 */
	...
	public final Geometry transform(final Geometry inputGeom) {
		...
		if (inputGeom instanceof MultiPolygon) { return transformMultiPolygon((MultiPolygon) inputGeom, null); }
	 ...
	 */
	protected final CoordinateSequence createCoordinateSequence(final Coordinate[] coords) {
	 ...
	 */
	protected final CoordinateSequence copy(final CoordinateSequence seq) {
		return seq.copy();
	}

	/**
	 * Transforms a {@link CoordinateSequence}. This method should always return a valid coordinate list for the desired
	 * result type. (E.g. a coordinate list for a LineString must have 0 or at least 2 points). If this is not possible,
	...
	protected Geometry transformMultiPoint(final MultiPoint geom, final Geometry parent) {
		...
		for (int i = 0; i < geom.getNumGeometries(); i++) {
			final Geometry transformGeom = transformPoint((Point) geom.getGeometryN(i), geom);
	...
	/**
	 * Transforms a LinearRing. The transformation of a LinearRing may result in a coordinate sequence which does not
	 * form a structurally valid ring (i.e. a degenerate ring of 3 or fewer points). In this case a LineString is
	 * returned. Subclasses may wish to override this method and check for this situation (e.g. a subclass may choose to
	 * eliminate degenerate linear rings)
	 *
	 * @param geom
	...
	protected Geometry transformLinearRing(final LinearRing geom, final Geometry parent) {
		final CoordinateSequence seq = transformCoordinates(geom.getCoordinateSequence(), geom);
	...
	protected Geometry transformMultiLineString(final MultiLineString geom, final Geometry parent) {
		final List<Geometry> transGeomList = new ArrayList<>();
		for (int i = 0; i < geom.getNumGeometries(); i++) {
			final Geometry transformGeom = transformLineString((LineString) geom.getGeometryN(i), geom);
			if (transformGeom == null) {
				continue;
			}
			if (transformGeom.isEmpty()) {
	...
	protected Geometry transformPolygon(final Polygon geom, final Geometry parent) {
		...
		final Geometry shell = transformLinearRing((LinearRing) geom.getExteriorRing(), geom);

		if (shell == null || !(shell instanceof LinearRing) || shell.isEmpty()) {
			isAllValidLinearRings = false;
		}

		final ArrayList<Geometry> holes = new ArrayList<>();
		for (int i = 0; i < geom.getNumInteriorRing(); i++) {
			final Geometry hole = transformLinearRing((LinearRing) geom.getInteriorRingN(i), geom);
		...
		if (isAllValidLinearRings) {
			return factory.createPolygon((LinearRing) shell, holes.toArray(new LinearRing[] {}));
		} else {
	...
	protected Geometry transformMultiPolygon(final MultiPolygon geom, final Geometry parent) {
		final List<Geometry> transGeomList = new ArrayList<>();
		for (int i = 0; i < geom.getNumGeometries(); i++) {
			final Geometry transformGeom = transformPolygon((Polygon) geom.getGeometryN(i), geom);
			if (transformGeom == null) {
				continue;
			}
			if (transformGeom.isEmpty()) {
	...
	protected Geometry transformGeometryCollection(final GeometryCollection geom, final Geometry parent) {
		final List<Geometry> transGeomList = new ArrayList<>();
		for (int i = 0; i < geom.getNumGeometries(); i++) {
			final Geometry transformGeom = transform(geom.getGeometryN(i));
			if (transformGeom == null) {
				continue;
			}
