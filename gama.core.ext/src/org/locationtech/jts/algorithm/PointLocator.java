...
/**
 * Computes the topological ({@link Location}) of a single point to a {@link Geometry}. A {@link BoundaryNodeRule} may
 * be specified to control the evaluation of whether the point lies on the boundary or not The default rule is to use
 * the the <i>SFS Boundary Determination Rule</i>
 * <p>
 * Notes:
 * <ul>
...
public class PointLocator {
	...
	/**
	 * Computes the topological relationship ({@link Location}) of a single point to a Geometry. It handles both
	 * single-element and multi-element Geometries. The algorithm for multi-part Geometries takes into account the SFS
	 * Boundary Determination Rule.
	 *
	 * @return the {@link Location} of the point relative to the input Geometry
	 */
	public int locate(final Coordinate p, final Geometry geom) {
		if (geom.isEmpty()) { return Location.EXTERIOR; }

		if (geom instanceof LineString) {
			return locateOnLineString(p, (LineString) geom);
		} else if (geom instanceof Polygon) { return locateInPolygon(p, (Polygon) geom); }
	...
	private void computeLocation(final Coordinate p, final Geometry geom) {
	...
	private int locateOnPoint(final Coordinate p, final Point pt) {
		// no point in doing envelope test, since equality test is just as fast

		final Coordinate ptCoord = pt.getCoordinate();
		if (ptCoord.equals2D(p)) { return Location.INTERIOR; }
		return Location.EXTERIOR;
	}

	private int locateOnLineString(final Coordinate p, final LineString l) {
		// bounding-box check
		if (!l.getEnvelopeInternal().intersects(p)) { return Location.EXTERIOR; }

		final CoordinateSequence seq = l.getCoordinateSequence();
		if (!l.isClosed()) {
	...
	private int locateInPolygonRing(final Coordinate p, final LinearRing ring) {
		// bounding-box check
	...
	private int locateInPolygon(final Coordinate p, final Polygon poly) {
