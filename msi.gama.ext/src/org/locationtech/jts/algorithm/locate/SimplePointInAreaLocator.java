...
/**
 * Computes the location of points relative to a {@link Polygonal} {@link Geometry}, using a simple <tt>O(n)</tt>
 * algorithm.
 * <p>
 * The algorithm used reports if a point lies in the interior, exterior, or exactly on the boundary of the Geometry.
 * <p>
 * Instance methods are provided to implement the interface {@link PointInAreaLocator}. However, they provide no
 * performance advantage over the class methods.
 * <p>
 * This algorithm is suitable for use in cases where only a few points will be tested. If many points will be tested,
...
public class SimplePointInAreaLocator implements PointOnGeometryLocator {
	 ...
	 */
	public static int locate(final Coordinate p, final Geometry geom) {
	...
	/**
	 * Determines whether a point is contained in a {@link Geometry}, or lies on its boundary. This is a convenience
	 * method for
	 * 
	 * <pre>
	 ...
	 */
	public static boolean isContained(final Coordinate p, final Geometry geom) {
		return Location.EXTERIOR != locate(p, geom);
	}

	private static int locateInGeometry(final Coordinate p, final Geometry geom) {
		if (geom instanceof Polygon) { return locatePointInPolygon(p, (Polygon) geom); }
		...
		if (geom instanceof GeometryCollection) {
			...
			while (geomi.hasNext()) {
				...
				if (g2 != geom) {
					final int loc = locateInGeometry(p, g2);
	 ...
	 *
	 */
	public static int locatePointInPolygon(final Coordinate p, final Polygon poly) {
		if (poly.isEmpty()) { return Location.EXTERIOR; }
		final LinearRing shell = (LinearRing) poly.getExteriorRing();
		final int shellLoc = locatePointInRing(p, shell);
		if (shellLoc != Location.INTERIOR) { return shellLoc; }

		// now test if the point lies in or on the holes
		for (int i = 0; i < poly.getNumInteriorRing(); i++) {
			final LinearRing hole = (LinearRing) poly.getInteriorRingN(i);
			final int holeLoc = locatePointInRing(p, hole);
	...
	/**
	 * Determines whether a point lies in a {@link Polygon}. If the point lies on the polygon boundary it is considered
	 * to be inside.
	 * 
	 * @param p
	 *            the point to test
	 ...
	 */
	public static boolean containsPointInPolygon(final Coordinate p, final Polygon poly) {
	 ...
	 */
	private static int locatePointInRing(final Coordinate p, final LinearRing ring) {
