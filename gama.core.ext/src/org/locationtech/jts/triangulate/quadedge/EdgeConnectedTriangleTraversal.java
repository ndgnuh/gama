...
public class EdgeConnectedTriangleTraversal {
	private final LinkedList<QuadEdgeTriangle> triQueue = new LinkedList<>();
	 ...
	 */
	public void init(final Collection<QuadEdgeTriangle> tris) {
		triQueue.addAll(tris);
	}

	/**
	 * Subclasses can call this method to add a triangle to the end of the queue. This is useful for initializing the
	 * queue to a chosen set of triangles.
	 *
	 * @param tri
	 *            a triangle
	 */
	...
	public void visitAll(final TraversalVisitor visitor) {
		while (!triQueue.isEmpty()) {
			final QuadEdgeTriangle tri = triQueue.removeFirst();
			process(tri, visitor);
		}
	}
