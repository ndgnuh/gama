 ...
 * <p>
 * <code>WKTReader</code> supports extracting <code>Geometry</code> objects from either {@link Reader}s or
 * {@link String}s. This allows it to function as a parser to read <code>Geometry</code> objects from text blocks
 * embedded in other data formats (e.g. XML).
 * <P>
 * <p>
 * A <code>WKTReader</code> is parameterized by a <code>GeometryFactory</code>, to allow it to create
 * <code>Geometry</code> objects of the appropriate implementation. In particular, the <code>GeometryFactory</code>
 * determines the <code>PrecisionModel</code> and <code>SRID</code> that is used.
 * <P>
 *
 * The <code>WKTReader</code> converts all input numbers to the precise internal representation.
 *
 * <h3>Notes:</h3>
 * <ul>
 * <li>Keywords are case-insensitive.
 * <li>The reader supports non-standard "LINEARRING" tags.
 * <li>The reader uses <tt>Double.parseDouble</tt> to perform the conversion of ASCII numbers to floating point. This
 * means it supports the Java syntax for floating point literals (including scientific notation).
 * </ul>
 *
 * <h3>Syntax</h3> The following syntax specification describes the version of Well-Known Text supported by JTS. (The
 * specification uses a syntax language similar to that used in the C and Java language specifications.)
 * <p>
 * As of version 1.15, JTS can read (but not write) WKT Strings including Z, M or ZM in the name of the geometry type
 * (ex. POINT Z, LINESTRINGZM). Note that it only makes the reader more flexible, but JTS could already read 3D
 * coordinates from WKT String and still can't read 4D coordinates.
 *
 * <blockquote>
 *
 * <pre>
...
public class WKTReader {
	...
	/**
	 * Sets a flag indicating, that coordinates may have 3 ordinate values even though no Z or M ordinate indicator is
	 * present. The default value is {@link #ALLOW_OLD_JTS_COORDINATE_SYNTAX}.
	 *
	 * @param value
	 *            a boolean value
	 */
	public void setIsOldJtsCoordinateSyntaxAllowed(final boolean value) {
		isAllowOldJtsCoordinateSyntax = value;
	}

	/**
	 * Sets a flag indicating, that point coordinates in a MultiPoint geometry must not be enclosed in paren. The
	 * default value is {@link #ALLOW_OLD_JTS_MULTIPOINT_SYNTAX}
	 *
	 * @param value
	 *            a boolean value
	 */
	public void setIsOldJtsMultiPointSyntaxAllowed(final boolean value) {
	 ...
	 *
	 * @param wellKnownText
	 *            one or more &lt;Geometry Tagged Text&gt; strings (see the OpenGIS Simple Features Specification)
	...
	public Geometry read(final Reader reader) throws ParseException {
		final StreamTokenizer tokenizer = createTokenizer(reader);
		try {
	 ...
	 */
	private static StreamTokenizer createTokenizer(final Reader reader) {
	 ...
	 * <p>
	 * All ordinate values are read, but -depending on the {@link CoordinateSequenceFactory} of the underlying
	 * {@link GeometryFactory}- not necessarily all can be handled. Those are silently dropped.
	 * </p>
	 *
	 * @param tokenizer
	 ...
	 */
	private CoordinateSequence getCoordinate(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags,
			final boolean tryParen) throws IOException, ParseException {
		...
		final int offsetM = ordinateFlags.contains(Ordinate.Z) ? 1 : 0;
		final CoordinateSequence sequence =
				csFactory.create(1, toDimension(ordinateFlags), ordinateFlags.contains(Ordinate.M) ? 1 : 0);
	 ...
	 *             if an unexpected token was encountered
	 */
	private CoordinateSequence getCoordinateSequence(final StreamTokenizer tokenizer,
			final EnumSet<Ordinate> ordinateFlags) throws IOException, ParseException {
	 ...
	 *            a bit-mask defining the ordinates to read.
	 * @param tryParen
	 *            a value indicating if a starting {@link #L_PAREN} should be probed for each coordinate.
	 * @return a {@link CoordinateSequence} of length 1 containing the read ordinate values
	 *
	 * @throws IOException
	 *             if an I/O error occurs
	 ...
	 */
	private CoordinateSequence getCoordinateSequence(final StreamTokenizer tokenizer,
			final EnumSet<Ordinate> ordinateFlags, final boolean tryParen) throws IOException, ParseException {
		...
		final ArrayList<CoordinateSequence> coordinates = new ArrayList<>();
		do {
	...
	/**
	 * Computes the required dimension based on the given ordinate values. It is assumed that {@link Ordinate#X} and
	 * {@link Ordinate#Y} are included.
	 *
	 * @param ordinateFlags
	 *            the ordinate bit-mask
	 * @return the number of dimensions required to store ordinates for the given bit-mask.
	 */
	private int toDimension(final EnumSet<Ordinate> ordinateFlags) {
		int dimension = 2;
		if (ordinateFlags.contains(Ordinate.Z)) {
			dimension++;
		}
	 ...
	 *
	 * @param sequences
	 *            an array of coordinate sequences. Each sequence contains <b>exactly one</b> coordinate.
	 * @param ordinateFlags
	 *            a bit-mask of required ordinates.
	 * @return a coordinate sequence containing all coordinate
	 */
	private CoordinateSequence mergeSequences(final ArrayList<CoordinateSequence> sequences,
			final EnumSet<Ordinate> ordinateFlags) {
		...
		final CoordinateSequence sequence = this.csFactory.create(sequences.size(), toDimension(mergeOrdinates),
				mergeOrdinates.contains(Ordinate.M) ? 1 : 0);

		final int offsetM = CoordinateSequence.Z + (mergeOrdinates.contains(Ordinate.Z) ? 1 : 0);
		for (int i = 0; i < sequences.size(); i++) {
			final CoordinateSequence item = sequences.get(i);
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next element returned by the stream
	 *            should be L_PAREN (the beginning of "(x1 y1, x2 y2, ..., xn yn)") or EMPTY.
	 ...
	 */
	@Deprecated
	private Coordinate[] getCoordinates(final StreamTokenizer tokenizer) throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals(EMPTY)) { return new Coordinate[] {}; }
		...
		final Coordinate[] array = new Coordinate[coordinates.size()];
	 ...
	 *            tokenizer over a stream of text in Well-known Text format. The next element returned by the stream
	 *            should be a number.
	 * @return the next array of <code>Coordinate</code>s in the stream.
	 * @throws IOException
	 *             if an I/O error occurs
	 * @throws ParseException
	 *             if an unexpected token was encountered
	 ...
	 */
	@Deprecated
	private Coordinate[] getCoordinatesNoLeftParen(final StreamTokenizer tokenizer) throws IOException, ParseException {
	 ...
	 */
	@Deprecated
	private Coordinate getPreciseCoordinate(final StreamTokenizer tokenizer) throws IOException, ParseException {
	 ...
	 *             if an I/O error occurs
	 */
	private static boolean isNumberNext(final StreamTokenizer tokenizer) throws IOException {
	 ...
	 *             if an I/O error occurs
	 */
	private static boolean isOpenerNext(final StreamTokenizer tokenizer) throws IOException {
		final int type = tokenizer.nextToken();
		tokenizer.pushBack();
		return type == '(';
	}
	 ...
	 * correctly, and the case of the "NaN" symbol is not significant.
	 *
	 * @param tokenizer
	 ...
	 *             if an I/O error occurs
	 */
	private double getNextNumber(final StreamTokenizer tokenizer) throws IOException, ParseException {
		final int type = tokenizer.nextToken();
		switch (type) {
	 ...
	 *            tokenizer over a stream of text in Well-known Text
	 */
	private static String getNextEmptyOrOpener(final StreamTokenizer tokenizer) throws IOException, ParseException {
	 ...
	 *            tokenizer over a stream of text in Well-known Text
	 */
	private static EnumSet<Ordinate> getNextOrdinateFlags(final StreamTokenizer tokenizer)
			throws IOException, ParseException {

		final EnumSet<Ordinate> result = EnumSet.of(Ordinate.X, Ordinate.Y);

		final String nextWord = lookAheadWord(tokenizer).toUpperCase();
		if (nextWord.equalsIgnoreCase("Z")) {
			tokenizer.nextToken();
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next token must be a word.
	 ...
	 *             if an I/O error occurs
	 */
	private static String lookAheadWord(final StreamTokenizer tokenizer) throws IOException, ParseException {
		final String nextWord = getNextWord(tokenizer);
		tokenizer.pushBack();
		return nextWord;
	}
	 ...
	 *            tokenizer over a stream of text in Well-known Text
	 */
	private static String getNextCloserOrComma(final StreamTokenizer tokenizer) throws IOException, ParseException {
		final String nextWord = getNextWord(tokenizer);
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next token must be R_PAREN.
	 ...
	 *            tokenizer over a stream of text in Well-known Text
	 */
	private static String getNextWord(final StreamTokenizer tokenizer) throws IOException, ParseException {
		final int type = tokenizer.nextToken();
		switch (type) {
			case StreamTokenizer.TT_WORD:
	 ...
	 */
	private static ParseException parseErrorExpected(final StreamTokenizer tokenizer, final String expected) {
		...
		final String tokenStr = tokenString(tokenizer);
	 ...
	 *             if an invalid token is encountered
	 */
	private static ParseException parseErrorWithLine(final StreamTokenizer tokenizer, final String msg) {
	 ...
	 */
	private static String tokenString(final StreamTokenizer tokenizer) {
	...
	private Geometry readGeometryTaggedText(final StreamTokenizer tokenizer) throws IOException, ParseException {
		...
		final EnumSet<Ordinate> ordinateFlags = EnumSet.of(Ordinate.X, Ordinate.Y);
		try {
	...
	private Geometry readGeometryTaggedText(final StreamTokenizer tokenizer, final String type,
		...
		try {
			...
			geometryFactory =
					new GeometryFactory(geometryFactory.getPrecisionModel(), geometryFactory.getSRID(), csFactoryXYZM);
		}
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next tokens must form a &lt;Point
	 *            Text&gt;.
	 ...
	 *             if an unexpected token was encountered
	 */
	private Point readPointText(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
		final Point point = geometryFactory.createPoint(getCoordinateSequence(tokenizer, ordinateFlags));
		return point;
	}
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next tokens must form a &lt;LineString
	 *            Text&gt;.
	 ...
	 *             if an unexpected token was encountered
	 */
	private LineString readLineStringText(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
	 ...
	 */
	private LinearRing readLinearRingText(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next tokens must form a &lt;MultiPoint
	 *            Text&gt;.
	 ...
	 *             if an unexpected token was encountered
	 */
	private MultiPoint readMultiPointText(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
		return geometryFactory
				.createMultiPoint(getCoordinateSequence(tokenizer, ordinateFlags, this.isAllowOldJtsMultipointSyntax));
	}
	 ...
	 *
	 * @param tokenizer
	 *            tokenizer over a stream of text in Well-known Text format. The next tokens must form a &lt;Polygon
	 *            Text&gt;.
	 ...
	 *             if an I/O error occurs
	 */
	private Polygon readPolygonText(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals(EMPTY)) { return geometryFactory.createPolygon(); }
		final ArrayList<LinearRing> holes = new ArrayList<>();
		final LinearRing shell = readLinearRingText(tokenizer, ordinateFlags);
		nextToken = getNextCloserOrComma(tokenizer);
		while (nextToken.equals(COMMA)) {
			final LinearRing hole = readLinearRingText(tokenizer, ordinateFlags);
			holes.add(hole);
			nextToken = getNextCloserOrComma(tokenizer);
		}
	 ...
	 *             if an unexpected token was encountered
	 */
	private MultiLineString readMultiLineStringText(final StreamTokenizer tokenizer,
			final EnumSet<Ordinate> ordinateFlags) throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals(EMPTY)) { return geometryFactory.createMultiLineString(); }

		final ArrayList<LineString> lineStrings = new ArrayList<>();
		do {
			final LineString lineString = readLineStringText(tokenizer, ordinateFlags);
	 ...
	 *             if an unexpected token was encountered
	 */
	private MultiPolygon readMultiPolygonText(final StreamTokenizer tokenizer, final EnumSet<Ordinate> ordinateFlags)
			throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals(EMPTY)) { return geometryFactory.createMultiPolygon(); }
		final ArrayList<Polygon> polygons = new ArrayList<>();
		do {
			final Polygon polygon = readPolygonText(tokenizer, ordinateFlags);
	 ...
	 *             if an I/O error occurs
	 */
	private GeometryCollection readGeometryCollectionText(final StreamTokenizer tokenizer,
			final EnumSet<Ordinate> ordinateFlags) throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals(EMPTY)) { return geometryFactory.createGeometryCollection(); }
		final ArrayList<Geometry> geometries = new ArrayList<>();
		do {
			final Geometry geometry = readGeometryTaggedText(tokenizer);
