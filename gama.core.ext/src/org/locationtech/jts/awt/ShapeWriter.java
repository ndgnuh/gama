...
/**
 * Writes {@link Geometry}s into Java2D {@link Shape} objects of the appropriate type. This supports rendering
 * geometries using Java2D. The ShapeWriter allows supplying a {@link PointTransformation} class, to transform
 * coordinates from model space into view space. This is useful if a client is providing its own transformation logic,
 * rather than relying on Java2D <tt>AffineTransform</tt>s.
 * <p>
 * The writer supports removing duplicate consecutive points (via the {@link #setRemoveDuplicatePoints(boolean)} method)
 * as well as true <b>decimation</b> (via the {@link #setDecimation(double)} method. Enabling one of these strategies
 * can substantially improve rendering speed for large geometries. It is only necessary to enable one strategy. Using
 * decimation is preferred, but this requires determining a distance below which input geometry vertices can be
 * considered unique (which may not always be feasible). If neither strategy is enabled, all vertices of the input
...
public class ShapeWriter {
	 ...
	 */
	public ShapeWriter(final PointTransformation pointTransformer, final PointShapeFactory pointFactory) {
	...
	/**
	 * Sets whether duplicate consecutive points should be eliminated. This can reduce the size of the generated Shapes
	 * and improve rendering speed, especially in situations where a transform reduces the extent of the geometry.
	 * <p>
	...
	/**
	 * Sets the decimation distance used to determine whether vertices of the input geometry are considered to be
	 * duplicate and thus removed. The distance is axis distance, not Euclidean distance. The distance is specified in
	 * the input geometry coordinate system (NOT the transformed output coordinate system).
	 * <p>
	 * When rendering to a screen image, a suitably small distance should be used to avoid obvious rendering defects. A
	 * distance equivalent to the equivalent of 1.5 pixels or less is recommended (and perhaps even smaller to avoid any
	...
	/**
	 * Creates a {@link Shape} representing a {@link Geometry}, according to the specified PointTransformation and
	 * PointShapeFactory (if relevant).
	 * <p>
	 * Note that Shapes do not preserve information about which elements in heterogeneous collections are 1D and which
	 * are 2D. For example, a GeometryCollection containing a ring and a disk will render as two disks if Graphics.fill
	 * is used, or as two rings if Graphics.draw is used. To avoid this issue use separate shapes for the components.
	 *
	 * @param geometry
	...
	private Shape toShape(final Polygon p) {
	...
	private void appendRing(final PolygonShape poly, final Coordinate[] coords) {
		...
		/**
		 * Don't include closing point. Ring path will be closed explicitly, which provides a more accurate path
		 * representation.
		 */
		for (int i = 0; i < n; i++) {

			if (decimationDistance > 0.0) {
				final boolean isDecimated = prev != null && Math.abs(coords[i].x - prev.x) < decimationDistance
			...
			if (doRemoveDuplicatePoints) {
				...
				final boolean isDup = transPoint.getX() == prevx && transPoint.getY() == prevy;
				if (i < n && isDup) {
					continue;
				}
	...
	private GeneralPath toShape(final MultiLineString mls) {
		...
		for (int i = 0; i < mls.getNumGeometries(); i++) {
			final LineString lineString = (LineString) mls.getGeometryN(i);
	...
	private GeneralPath toShape(final LineString lineString) {
		...
		final int n = lineString.getNumPoints() - 1;
		// int count = 0;
		for (int i = 1; i <= n; i++) {
			final Coordinate currentCoord = lineString.getCoordinateN(i);
			if (decimationDistance > 0.0) {
				final boolean isDecimated = prev != null && Math.abs(currentCoord.x - prev.x) < decimationDistance
	...
	private Point2D transformPoint(final Coordinate model) {
		return transformPoint(model, new Point2D.Double());
	}

	private Point2D transformPoint(final Coordinate model, final Point2D view) {
