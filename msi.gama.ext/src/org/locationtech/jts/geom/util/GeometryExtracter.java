...
public class GeometryExtracter implements GeometryFilter {
	...
	protected static boolean isOfClass(final Object o, final Class<?> clz) {
		return clz.isAssignableFrom(o.getClass());
		// return o.getClass() == clz;
	}

	/**
	 * Extracts the components of type <tt>clz</tt> from a {@link Geometry} and adds them to the provided {@link List}.
	 * 
	 * @param geom
	 ...
	 */
	public static List<Geometry> extract(final Geometry geom, final Class<?> clz, final List<Geometry> list) {
	...
	/**
	 * Extracts the components of type <tt>clz</tt> from a {@link Geometry} and returns them in a {@link List}.
	 * 
	 * @param geom
	 *            the geometry from which to extract
	 */
	...
	private final List<Geometry> comps;
	 ...
	 */
	public GeometryExtracter(final Class<?> clz, final List<Geometry> comps) {
		this.clz = clz;
		this.comps = comps;
	}
	...
	public void filter(final Geometry geom) {
		if (clz == null || isOfClass(geom, clz)) {
			comps.add(geom);
		}
	}
