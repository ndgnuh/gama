/*******************************************************************************************************
 *
 * gaml.operators.GraphsGraphstream.java, in plugin gama.core, is part of the source code of the GAMA modeling
...
import gama.core.ext.graphstream.BarabasiAlbertGenerator;
import gama.core.ext.graphstream.BaseGenerator;
import gama.core.ext.graphstream.FullGenerator;
import gama.core.ext.graphstream.Sink;
import gama.core.ext.graphstream.SinkAdapter;
import gama.core.ext.graphstream.WattsStrogatzGenerator;
...
import gama.util.graph.GraphAndPopulationsSynchronizer;
...
public class GraphsGraphstream {
	...
	/**
	 * Receives events from a graphstream loader and updates the GAMA Igraph accordingly
	 *
	...
	public static class GraphStreamGamaGraphSink extends SinkAdapter {
		...
		public void edgeAdded(final String sourceId, final long timeId, final String edgeId, final String fromNodeId,
			...
			final IShape geom = GamaGeometryType.buildLink(scope, agentFrom, agentTo);
			initialValues.get(0).put(IKeyword.SHAPE, geom);
			// initialValues.get(0).put(IKeyword.LOCATION, geom.getLocation());
			final IList<? extends IAgent> createdAgents =
					populationEdges.createAgents(scope, 1, initialValues, false, true);
	...
	/**
	 * Loads the graph from the graphstream generator passed as parameter and according to the parameters passed from
	 * GAMA. Note that the generator is supposed to be already configured.
	 *
	 * @param scope
	 * @param params
	 * @param generator
