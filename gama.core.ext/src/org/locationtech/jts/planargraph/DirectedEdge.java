...
/**
 * Represents a directed edge in a {@link PlanarGraph}. A DirectedEdge may or may not have a reference to a parent
 * {@link Edge} (some applications of planar graphs may not require explicit Edge objects to be created). Usually a
 * client using a <code>PlanarGraph</code> will subclass <code>DirectedEdge</code> to add its own application-specific
 * data and methods.
 *
 * @version 1.7
 */
public class DirectedEdge extends GraphComponent implements Comparable {
	 ...
	 *
	 * @param directionPt
	 *            specifies this DirectedEdge's direction vector (determined by the vector from the <code>from</code>
	 *            node to <code>directionPt</code>)
	 * @param edgeDirection
	 *            whether this DirectedEdge's direction is the same as or opposite to that of the parent Edge (if any)
	 */
	public DirectedEdge(final Node from, final Node to, final Coordinate directionPt, final boolean edgeDirection) {
		this.from = from;
		this.to = to;
	...
	/**
	 * Returns 1 if this DirectedEdge has a greater angle with the positive x-axis than b", 0 if the DirectedEdges are
	 * collinear, and -1 otherwise.
	 * <p>
	 * Using the obvious algorithm of simply computing the angle is not robust, since the angle calculation is
	 * susceptible to roundoff. A robust algorithm is:
	 * <ul>
	 * <li>first compare the quadrants. If the quadrants are different, it it trivial to determine which vector is
	 * "greater".
	 * <li>if the vectors lie in the same quadrant, the robust
	 * {@link Orientation#computeOrientation(Coordinate, Coordinate, Coordinate)} function can be used to decide the
