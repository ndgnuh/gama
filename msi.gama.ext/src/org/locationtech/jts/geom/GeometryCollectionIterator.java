 ...
 * The iteration sequence follows a pre-order, depth-first traversal of the structure of the
 * <code>GeometryCollection</code> (which may be nested). The original <code>Geometry</code> object is returned as well
 * (as the first object), as are all sub-collections and atomic elements. It is simple to ignore the intermediate
...
public class GeometryCollectionIterator implements Iterator {
	...
	/**
	 * Indicates whether or not the first element (the root <code>GeometryCollection</code>) has been returned.
	 */
	 ...
	 * <code>GeometryCollectionIterator</code> is not currently iterating over a nested <code>GeometryCollection</code>.
	 */
	...
	public Object next() {
		...
		final Geometry obj = parent.getGeometryN(index++);
	...
	private static boolean isAtomic(final Geometry geom) {
