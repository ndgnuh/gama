...
/**
 * A SAX {@link DefaultHandler} which builds {@link Geometry}s from GML2-formatted geometries. An XML parser can
 * delegate SAX events to this handler to parse and building Geometrys.
 * <p>
...
public class GMLHandler extends DefaultHandler {
	...
	/**
	 * This class is intended to log the SAX activity within a given element until its termination. At this time, a new
	 * object of value is created and passed to the parent. An object of value is typically either java.lang.* or a JTS
	 * Geometry This class is not intended for use outside this distribution, and may change in subsequent versions.
	 *
	 * @author David Zwiers, Vivid Solutions.
	 */
	static class Handler {
		 ...
		 *            Nullable
		 */
		public Handler(final ParseStrategy strategy, final Attributes attributes) {
			if (attributes != null) {
		 ...
		 * @param obj
		 */
		public void keep(final Object obj) {
			if (children == null) {
		 ...
		 * @throws SAXException
		 */
		public Object create(final GeometryFactory gf) throws SAXException {
			return strategy.parse(this, gf);
		}
	}

	private final Stack<Handler> stack = new Stack<>();
	...
	/**
	 * Creates a new handler. Allows the user to specify a delegate object for error / warning messages. If the delegate
	 * also implements ContentHandler then the document Locator will be passed on.
	 *
	 * @param gf
	 *            Geometry Factory
	 * @param delegate
	 ...
	 *
	 */
	public GMLHandler(final GeometryFactory gf, final ErrorHandler delegate) {
	...
	/**
	 * Tests whether this handler has completed parsing a geometry. If this is the case, {@link #getGeometry()} can be
	 * called to get the value of the parsed geometry.
	 *
	...
	/**
	 * Gets the geometry parsed by this handler. This method should only be called AFTER the parser has completed
	 * execution
	 *
	...
	public Geometry getGeometry() {
		if (stack.size() == 1) {
			...
			return gf.createGeometryCollection(h.children.toArray(new Geometry[stack.size()]));
		}
	...
	public void endElement(final String uri, final String localName, final String qName) throws SAXException {
		final Handler thisAction = stack.pop();
	...
	public void startElement(final String uri, final String localName, final String qName, final Attributes attributes)
		...
		if (ps == null) {
			final String qn = qName.substring(qName.indexOf(':') + 1, qName.length());
			ps = GeometryStrategies.findStrategy(null, qn);
		}
		final Handler h = new Handler(ps, attributes);
