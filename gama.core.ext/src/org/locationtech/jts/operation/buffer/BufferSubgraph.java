 ...
 * @version 1.7
 */
class BufferSubgraph implements Comparable<Object> {
	...
	public Envelope getEnvelope() {
		if (env == null) {
			...
			for (final DirectedEdge directedEdge : dirEdgeList) {
				final DirectedEdge dirEdge = directedEdge;
	...
	/**
	 * Creates the subgraph consisting of all edges reachable from this node. Finds the edges in the graph and the
	...
	private void add(final Node node, final Stack<Node> nodeStack) {
		...
		for (final Iterator<?> i = ((DirectedEdgeStar) node.getEdges()).iterator(); i.hasNext();) {
			...
			/**
			 * NOTE: this is a depth-first traversal of the graph. This will cause a large depth of recursion. It might
			 * be better to do a breadth-first traversal.
			 */
	...
	/**
	 * Find all edges whose depths indicates that they are in the result area(s). Since we want polygon shells to be
	 * oriented CW, choose dirEdges with the interior of the result on the RHS. Mark them as being in the result.
	 * Interior Area edges are the result of dimensional collapses. They do not form part of the result area boundary.
	 */
	public void findResultEdges() {
		for (final DirectedEdge directedEdge : dirEdgeList) {
			final DirectedEdge de = directedEdge;
			/**
			 * Select edges which have an interior depth on the RHS and an exterior depth on the LHS. Note that because
			 * of weird rounding effects there may be edges which have negative depths! Negative depths count as
			 * "outside".
			 */
	...
	/**
	 * BufferSubgraphs are compared on the x-value of their rightmost Coordinate. This defines a partial ordering on the
	 * graphs such that:
	 * <p>
	...
	/*
	 * // DEBUGGING only - comment out private static final String SAVE_DIREDGES = "saveDirEdges"; private static int
	 * saveCount = 0; public void saveDirEdges() { GeometryFactory fact = new GeometryFactory(); for (Iterator it =
	 * dirEdgeList.iterator(); it.hasNext(); ) { DirectedEdge de = (DirectedEdge) it.next(); double dx = de.getDx();
	 * double dy = de.getDy(); Coordinate p0 = de.getCoordinate(); double ang = Math.atan2(dy, dx); Coordinate p1 = new
	 * Coordinate( p0.x + .4 * Math.cos(ang), p0.y + .4 * Math.sin(ang)); // DebugFeature.add(SAVE_DIREDGES, //
	 * fact.createLineString(new Coordinate[] { p0, p1 } ), // de.getDepth(Position.LEFT) + "/" +
	 * de.getDepth(Position.RIGHT) // ); } String filepath = "x:\\jts\\testBuffer\\dirEdges" + saveCount++ + ".jml";
