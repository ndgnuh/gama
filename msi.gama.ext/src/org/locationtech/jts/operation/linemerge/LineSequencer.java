...
/**
 * Builds a sequence from a set of LineStrings so that they are ordered end to end. A sequence is a complete
 * non-repeating list of the linear components of the input. Each linestring is oriented so that identical endpoints are
 * adjacent in the list.
 * <p>
 * A typical use case is to convert a set of unoriented geometric links from a linear network (e.g. such as block faces
 * on a bus route) into a continuous oriented path through the network.
 * <p>
 * The input linestrings may form one or more connected sets. The input linestrings should be correctly noded, or the
 * results may not be what is expected. The computed output is a single {@link MultiLineString} containing the ordered
 * linestrings in the sequence.
 * <p>
 * The sequencing employs the classic <b>Eulerian path</b> graph algorithm. Since Eulerian paths are not uniquely
 * determined, further rules are used to make the computed sequence preserve as much as possible of the input ordering.
 * Within a connected subset of lines, the ordering rules are:
 * <ul>
 * <li>If there is degree-1 node which is the start node of an linestring, use that node as the start of the sequence
 * <li>If there is a degree-1 node which is the end node of an linestring, use that node as the end of the sequence
 * <li>If the sequence has no degree-1 nodes, use any node as the start
 * </ul>
 *
 * Note that not all arrangements of lines can be sequenced. For a connected set of edges in a graph, <i>Euler's
 * Theorem</i> states that there is a sequence containing each edge once <b>if and only if</b> there are no more than 2
 * nodes of odd degree. If it is not possible to find a sequence, the {@link #isSequenceable()} method will return
 * <code>false</code>.
 *
 * @version 1.7
 */
public class LineSequencer {
	...
	/**
	 * Tests whether a {@link Geometry} is sequenced correctly. {@link LineString}s are trivially sequenced.
	 * {@link MultiLineString}s are checked for correct sequencing. Otherwise, <code>isSequenced</code> is defined to be
	 * <code>true</code> for geometries that are not lineal.
	 *
	 * @param geom
	...
	public static boolean isSequenced(final Geometry geom) {
		...
		final Set<Coordinate> prevSubgraphNodes = new TreeSet<>();

		Coordinate lastNode = null;
		final List<Coordinate> currNodes = new ArrayList<>();
		for (int i = 0; i < mls.getNumGeometries(); i++) {
			final LineString line = (LineString) mls.getGeometryN(i);
			final Coordinate startNode = line.getCoordinateN(0);
			final Coordinate endNode = line.getCoordinateN(line.getNumPoints() - 1);
	...
	/**
	 * Adds a {@link Collection} of {@link Geometry}s to be sequenced. May be called multiple times. Any dimension of
	...
	public void add(final Collection<?> geometries) {
		for (final Object name : geometries) {
			final Geometry geometry = (Geometry) name;
			add(geometry);
		}
	}

	/**
	 * Adds a {@link Geometry} to be sequenced. May be called multiple times. Any dimension of Geometry may be added;
	...
	/**
	 * Returns the {@link LineString} or {@link MultiLineString} built by the sequencing process, if one exists.
	 *
	...
	private void computeSequence() {
		...
		final List<List<DirectedEdge>> sequences = findSequences();
	...
	private List<List<DirectedEdge>> findSequences() {
		final List<List<DirectedEdge>> sequences = new ArrayList<>();
		...
		for (final Object name : subgraphs) {
			final Subgraph subgraph = (Subgraph) name;
			if (hasSequence(subgraph)) {
				final List<DirectedEdge> seq = findSequence(subgraph);
				sequences.add(seq);
			} else {
	...
	private List<DirectedEdge> findSequence(final Subgraph graph) {
		...
		final List<DirectedEdge> seq = new LinkedList<>();
		final ListIterator<DirectedEdge> lit = seq.listIterator();
		addReverseSubpath(startDESym, lit, false);
		while (lit.hasPrevious()) {
			final DirectedEdge prev = lit.previous();
		...
		/**
		 * At this point, we have a valid sequence of graph DirectedEdges, but it is not necessarily appropriately
		 * oriented relative to the underlying geometry.
		 */
		final List<DirectedEdge> orientedSeq = orient(seq);
		return orientedSeq;
	}

	/**
	 * Finds an {@link DirectedEdge} for an unvisited edge (if any), choosing the dirEdge which preserves orientation,
	 * if possible.
	 *
	 * @param node
	 *            the node to examine
	...
	private void addReverseSubpath(DirectedEdge de, final ListIterator<DirectedEdge> lit,
	 ...
	 * Heuristics used are:
	 * <ul>
	 * <li>If the path has a degree-1 node which is the start node of an linestring, use that node as the start of the
	 * sequence
	 * <li>If the path has a degree-1 node which is the end node of an linestring, use that node as the end of the
	 * sequence
	 * <li>If the sequence has no degree-1 nodes, use any node as the start (NOTE: in this case could orient the
	 * sequence according to the majority of the linestring orientations)
	 * </ul>
	 *
	 * @param seq
	 ...
	 */
	private List<DirectedEdge> orient(final List<DirectedEdge> seq) {
		final DirectedEdge startEdge = seq.get(0);
		final DirectedEdge endEdge = seq.get(seq.size() - 1);
	 ...
	 */
	private List<DirectedEdge> reverse(final List<DirectedEdge> seq) {
		final LinkedList<DirectedEdge> newSeq = new LinkedList<>();
		for (final DirectedEdge directedEdge : seq) {
			final DirectedEdge de = directedEdge;
	 ...
	 *
	 * @param sequences
	 *            a List of Lists of DirectedEdges with LineMergeEdges as their parent edges.
	 * @return the sequenced geometry, or <code>null</code> if no sequence exists
	 */
	private Geometry buildSequencedGeometry(final List<List<DirectedEdge>> sequences) {
		final List<LineString> lines = new ArrayList<>();

		for (final List<DirectedEdge> list : sequences) {
			final List<?> seq = list;
			for (final Object name : seq) {
				final DirectedEdge de = (DirectedEdge) name;
		...
		if (lines.size() == 0) { return factory.createMultiLineString(new LineString[0]); }
	...
	private static LineString reverse(final LineString line) {
		...
		final Coordinate[] revPts = new Coordinate[pts.length];
