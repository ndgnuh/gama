...
public class Bound extends Entity implements Comparable<Bound> {
	 ...
	 *
	 */
	public Bound(final String origin) {
	 ...
	 *            The origin (source) of the data, typically a URI
	 */
	public Bound(final double right, final double left, final double top, final double bottom, final String origin) {
		...
		if (Double.compare(top, bottom) < 0) { throw new IllegalArgumentException("Bound top < bottom"); }
		this.right = right;
		this.left = left;
		this.top = top;
		this.bottom = bottom;
	 ...
	 */
	public Bound(final StoreReader sr, final StoreClassRegister scr) {
		super(sr, scr);
	 ...
	 */
	public Bound intersect(final Bound intersectingBound) {
		String newOrigin;
		...
		if (intersect180 && this180 || !(intersect180 || this180)) {
			...
			if (!(intersect180 || this180) && Double.compare(newLeft, newRight) >= 0) {
				/*
				 * This is only applicable for the case where neither cross the antimeridian, because if both cross,
				 * they must intersect.
				 */
			...
			if (Double.compare(b1.getRight(), b2.getLeft()) > 0 && Double.compare(b1.getLeft(), b2.getRight()) < 0) {
				...
				// two
				final Double diff1 = b1.getRight() - b1.getLeft();
				final Double diff2 = b2.getRight() - MIN_LONGITUDE + MAX_LONGITUDE - b2.getLeft();
		...
		if (Double.compare(newRight, newLeft) == 0) { return null; }
	...
	/**
	 * Calculate the union area of this with the specified bound. Not a strict mathematical union, but the smallest
	 * rectangular area which includes both bound. Thus, result may include areas not contained in the original bound.
	 *
	 * @param unionBound
	 *            Bound element with which to calculate the union
	 * @return Bound Resultant union of the two bound objects
	 */
	public Bound union(final Bound unionBound) {
		double newRight = 0.0, newLeft = 0.0, newTop, newBottom;
		String newOrigin;
		...
		if (Double.compare(this.getLeft(), MIN_LONGITUDE) == 0 && Double.compare(this.getRight(), MAX_LONGITUDE) == 0
				|| Double.compare(unionBound.getLeft(), MIN_LONGITUDE) == 0
						&& Double.compare(unionBound.getRight(), MAX_LONGITUDE) == 0) {
			...
			if (union180 && this180) {
				...
				if (Double.compare(b1.getRight(), b2.getLeft()) >= 0
					...
					size1 = Math.max(b1.getRight(), b2.getRight()) - MIN_LONGITUDE + (MAX_LONGITUDE - b1.getLeft());
					// first calculate the size of a union with the simple bound added to the right
					size2 = b1.getRight() - MIN_LONGITUDE + (MAX_LONGITUDE - Math.min(b1.getLeft(), b2.getLeft()));
	...
	/**
	 * Retrieve a collection of Bound objects which collectively comprise the entirety of this Bound but individually do
	 * not cross the antimeridian and thus can be used in simple area operations. The degenerate case will return this
	 * Bound.
	 *
	 * @return Iterable collection of Bound elements
	 */
	public Iterable<Bound> toSimpleBound() {
		final Collection<Bound> c = new LinkedList<>();
		if (Double.compare(this.getLeft(), this.getRight()) < 0) {
			...
			c.add(new Bound(MAX_LONGITUDE, this.getLeft(), this.getTop(), this.getBottom(), this.getOrigin()));
			c.add(new Bound(this.getRight(), MIN_LONGITUDE, this.getTop(), this.getBottom(), this.getOrigin()));
		}
		return Collections.unmodifiableCollection(c);
	}

	/**
	 * Compares this bound to the specified bound. The bound comparison is based on a comparison of area, latitude, and
	 * longitude in that order.
	 *
	 * @param comparisonBound
	 *            The bound to compare to.
	 * @return 0 if equal, &lt; 0 if this sorts before comparison (this is "smaller"), and &gt; 0 if this sorts before
	 *         comparison (this is "bigger")
	 */
	@Override
	public int compareTo(final Bound comparisonBound) {
		double areaT = 0.0, areaC = 0.0;
		int result;

		/*
		 * This is a very simple "area" calculation just using the coordinate values, not accounting for any
		 * projections.
		 */
		for (final Bound b : this.toSimpleBound()) {
			areaT += (b.getRight() - b.getLeft()) * (b.getTop() - b.getBottom());
		}
		for (final Bound b : comparisonBound.toSimpleBound()) {
		...
		final String myOrigin = this.getOrigin();
		final String otherOrigin = comparisonBound.getOrigin();
