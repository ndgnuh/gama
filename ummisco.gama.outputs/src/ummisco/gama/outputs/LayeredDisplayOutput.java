/*******************************************************************************************************
 *
 * ummisco.gama.outputs.LayeredDisplayOutput.java, in plugin msi.gama.core, is part of the source code of the GAMA
...
import ummisco.gama.outputs.LayeredDisplayOutput.DisplaySerializer;
import ummisco.gama.outputs.LayeredDisplayOutput.InfoValidator;
import ummisco.gama.outputs.layers.AbstractLayerStatement;
import ummisco.gama.outputs.layers.OverlayStatement;
				...
				@usage (
						...
						examples = { @example (
								...
								@example (
										value = "}",
										isExecutable = false) }) })
public class LayeredDisplayOutput extends AbstractDisplayOutput implements IDisplayOutput.Layered {

	private final List<ILayerStatement> layers;
	protected IDisplaySurface surface;
	private int index;

	final IDisplayData data = new LayeredDisplayData();
	...
	public static class InfoValidator implements IDescriptionValidator<IDescription> {
		...
		public void validate(final IDescription d) {
			...
			final Boolean isOpenGLWanted =
					type == null ? isOpenGLDefault : type.getExpression().literalValue().equals(IDisplayData.OPENGL);
			if (type != null) {
				...
				if (isOpenGLDefault) {
					d.setFacet(TYPE, LabelExpressionDescription.create(IDisplayData.OPENGL));
				}
			}
	...
	@Override
	public IOverlayProvider<IOverlayInfo> getOverlayProvider() {
		return overlayInfo;
	}
	...
	@Override
	public List<ILayerStatement> getLayers() {
		return layers;
	}
