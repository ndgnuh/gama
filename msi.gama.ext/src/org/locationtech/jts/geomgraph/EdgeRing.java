...
public abstract class EdgeRing {
	...
	private final List<DirectedEdge> edges = new ArrayList<>(); // the DirectedEdges making up this EdgeRing
	private final List<Coordinate> pts = new ArrayList<>();
	...
	private final ArrayList<EdgeRing> holes = new ArrayList<>(); // a list of EdgeRings which are holes in this
																	// EdgeRing
	...
	public Polygon toPolygon(final GeometryFactory geometryFactory) {
		final LinearRing[] holeLR = new LinearRing[holes.size()];
		for (int i = 0; i < holes.size(); i++) {
			holeLR[i] = holes.get(i).getLinearRing();
		}
		final Polygon poly = geometryFactory.createPolygon(getLinearRing(), holeLR);
		return poly;
	}

	/**
	 * Compute a LinearRing from the point list previously collected. Test if the ring is a hole (i.e. if it is CCW) and
	 * set the hole flag accordingly.
	 */
	public void computeRing() {
		if (ring != null) {
			return; // don't compute more than once
		}
		final Coordinate[] coord = new Coordinate[pts.size()];
	...
	/**
	 * Merge the RHS label from a DirectedEdge into the label for this EdgeRing. The DirectedEdge label may be null.
	 * This is acceptable - it results from a node which is NOT an intersection node between the Geometries (e.g. the
	 * end node of a LinearRing). In this case the DirectedEdge label does not contribute any information to the overall
	 * labelling, and is simply skipped.
	 */
	 ...
	 */
	public boolean containsPoint(final Coordinate p) {
		final LinearRing shell = getLinearRing();
		final Envelope env = shell.getEnvelopeInternal();
		if (!env.contains(p)) { return false; }
		if (!PointLocation.isInRing(p, shell.getCoordinates())) { return false; }

		for (final EdgeRing edgeRing : holes) {
			final EdgeRing hole = edgeRing;
