...
/**
 * Implements a "hot pixel" as used in the Snap Rounding algorithm. A hot pixel contains the interior of the tolerance
 * square and the boundary <b>minus</b> the top and right segments.
 * <p>
...
public class HotPixel {
	 ...
	 * 
	 */
	public HotPixel(final Coordinate pt, final double scaleFactor, final LineIntersector li) {
		originalPt = pt;
		this.pt = pt;
	...
	/**
	 * Returns a "safe" envelope that is guaranteed to contain the hot pixel. The envelope returned will be larger than
	 * the exact envelope of the pixel.
	 * 
	...
	private void initCorners(final Coordinate pt) {
	...
	private void copyScaled(final Coordinate p, final Coordinate pScaled) {
		pScaled.x = scale(p.x);
		pScaled.y = scale(p.y);
	}

	private boolean intersectsScaled(final Coordinate p0, final Coordinate p1) {
		final double segMinx = Math.min(p0.x, p1.x);
		final double segMaxx = Math.max(p0.x, p1.x);
		final double segMiny = Math.min(p0.y, p1.y);
		final double segMaxy = Math.max(p0.y, p1.y);

		final boolean isOutsidePixelEnv = maxx < segMinx || minx > segMaxx || maxy < segMiny || miny > segMaxy;
		...
		/*
		 * if (scaleFactor != 1.0) { boolean intersectsScaled = intersectsScaledTest(p0, p1); if (intersectsScaled !=
		 * intersects) { intersectsScaledTest(p0, p1); //
		 * Debug.println("Found hot pixel scaled intersection mismatch at " + pt); // Debug.println("Test segment: " +
		 * p0 + " " + p1); } return intersectsScaled; }
		 */
	...
	/**
	 * Tests whether the segment p0-p1 intersects the hot pixel tolerance square. Because the tolerance square point set
	 * is partially open (along the top and right) the test needs to be more sophisticated than simply checking for any
	 * intersection. However, it can take advantage of the fact that the hot pixel edges do not lie on the coordinate
	 * grid. It is sufficient to check if any of the following occur:
	 * <ul>
	 * <li>a proper intersection between the segment and any hot pixel edge
	 * <li>an intersection between the segment and <b>both</b> the left and bottom hot pixel edges (which detects the
	 * case where the segment intersects the bottom left hot pixel corner)
	 ...
	 * @return
	 */
	private boolean intersectsToleranceSquare(final Coordinate p0, final Coordinate p1) {
	...
	/**
	 * Test whether the given segment intersects the closure of this hot pixel. This is NOT the test used in the
	 * standard snap-rounding algorithm, which uses the partially closed tolerance square instead. This routine is
	 ...
	 */
	private boolean intersectsPixelClosure(final Coordinate p0, final Coordinate p1) {
		li.computeIntersection(p0, p1, corner[0], corner[1]);
	...
	public boolean addSnappedNode(final NodedSegmentString segStr, final int segIndex) {
		final Coordinate p0 = segStr.getCoordinate(segIndex);
		final Coordinate p1 = segStr.getCoordinate(segIndex + 1);
