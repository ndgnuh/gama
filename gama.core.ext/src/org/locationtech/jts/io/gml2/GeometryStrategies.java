...
public class GeometryStrategies {
	 ...
	 *
	 * The implementation of this class are intended to be used as static function points in C. These strategies should
	 * be associated with an element when the element begins. The strategy is utilized at the end of the element to
	 * create an object of value to the user.
	 *
	...
	private static HashMap<String, ParseStrategy> loadStrategies() {
		final HashMap<String, ParseStrategy> strats = new HashMap<>();

		// point
		strats.put(GMLConstants.GML_POINT.toLowerCase(), (arg, gf) -> {
			...
			final int srid = getSrid(arg.attrs, gf.getSRID());

			final Object c = arg.children.get(0);
			Point p = null;
		...
		// linestring
		strats.put(GMLConstants.GML_LINESTRING.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			if (arg.children.size() == 1) {
				// coord set
				try {
					final CoordinateSequence cs = (CoordinateSequence) arg.children.get(0);
				...
				try {
					final Coordinate[] coords = arg.children.toArray(new Coordinate[arg.children.size()]);
		...
		// linearring
		strats.put(GMLConstants.GML_LINEARRING.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
		...
		// polygon
		strats.put(GMLConstants.GML_POLYGON.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			final LinearRing outer = (LinearRing) arg.children.get(0); // will be the first
			final List<?> t = arg.children.size() > 1 ? arg.children.subList(1, arg.children.size()) : null;
			final LinearRing[] inner = t == null ? null : (LinearRing[]) t.toArray(new LinearRing[t.size()]);

			final Polygon p = gf.createPolygon(outer, inner);
		...
		// box
		strats.put(GMLConstants.GML_BOX.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
		...
		// multi-point
		strats.put(GMLConstants.GML_MULTI_POINT.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			final Point[] pts = arg.children.toArray(new Point[arg.children.size()]);

			final MultiPoint mp = gf.createMultiPoint(pts);
		...
		// multi-linestring
		strats.put(GMLConstants.GML_MULTI_LINESTRING.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			final LineString[] lns = arg.children.toArray(new LineString[arg.children.size()]);

			final MultiLineString mp = gf.createMultiLineString(lns);
		...
		// multi-poly
		strats.put(GMLConstants.GML_MULTI_POLYGON.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			final Polygon[] plys = arg.children.toArray(new Polygon[arg.children.size()]);

			final MultiPolygon mp = gf.createMultiPolygon(plys);
		...
		// multi-geom
		strats.put(GMLConstants.GML_MULTI_GEOMETRY.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			final Geometry[] geoms = arg.children.toArray(new Geometry[arg.children.size()]);

			final GeometryCollection gc = gf.createGeometryCollection(geoms);
		...
		strats.put(GMLConstants.GML_COORDINATES.toLowerCase(), new ParseStrategy() {
			...
			@Override
			public Object parse(final Handler arg, final GeometryFactory gf) throws SAXException {
				// one child, either a coord
				// or a coordinate sequence
				...
				final String[] touples = ptn.split(t.trim());// t.trim().split(toupleSeperator);
				...
				final CoordinateSequence cs = gf.getCoordinateSequenceFactory().create(numNonNullTouples, dim);
				dim = cs.getDimension(); // max dim

				final boolean replaceDec = !".".equals(decimal);
				...
				for (int i = 0; i < numNonNullTouples; i++) {
					...
					final String[] coords = ptn.split(touples[i]);// touples[i].split(coordSeperator);

					int dimIndex = 0;
					for (int j = 0; j < coords.length && j < dim; j++) {
						if (coords[j] != null && !"".equals(coords[j].trim())) {
							final double ordinate =
									Double.parseDouble(replaceDec ? coords[j].replaceAll(decimal, ".") : coords[j]);
		...
		// coord
		strats.put(GMLConstants.GML_COORD.toLowerCase(), (arg, gf) -> {
			// one child, either a coord
			// or a coordinate sequence
			...
			final Double[] axis = arg.children.toArray(new Double[arg.children.size()]);
		...
		final ParseStrategy member = (arg, gf) -> {
	...
	static int getSrid(final Attributes attrs, final int defaultValue) {
		String srs = null;
	 ...
	 * @see ParseStrategy
	 */
	public static ParseStrategy findStrategy(final String uri, final String localName) {
