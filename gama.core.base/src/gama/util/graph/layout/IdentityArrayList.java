/*
 * Copyright (c) 2007, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR
 * THIS FILE HEADER.
 *
...
/**
 * Resizable-array implementation of the <tt>List interface. Implements all optional list operations, and permits all
 * elements, including <tt>null. In addition to implementing the List interface, this class provides methods to
 ...
 * <p>
 *
 * An application can increase the capacity of an <tt>IdentityArrayList instance before adding a large number of
 * elements using the <tt>ensureCapacity operation. This may reduce the amount of incremental reallocation.
 *
 * <p>
 * Note that this implementation is not synchronized. If multiple threads access an <tt>IdentityArrayList instance
 * concurrently, and at least one of the threads modifies the list structurally, it <i>must be synchronized externally.
 ...
 *
 * <p>
 * The iterators returned by this class's iterator and <tt>listIterator methods are fail-fast: if the list is
 * structurally modified at any time after the iterator is created, in any way except through the iterator's own
 * <tt>remove or add methods, the iterator will throw a {@link ConcurrentModificationException}. Thus, in the face of
 ...
 * <tt>ConcurrentModificationException on a best-effort basis. Therefore, it would be wrong to write a program that
...
public class IdentityArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess {
	...
	/**
	 * Trims the capacity of this <tt>IdentityArrayList instance to be the list's current size. An application can use
	 * this operation to minimize the storage of an <tt>IdentityArrayList instance.
	 */
	...
	/**
	 * Increases the capacity of this <tt>IdentityArrayList instance, if necessary, to ensure that it can hold at least
	...
	/**
	 * Returns <tt>true if this list contains the specified element. More formally, returns <tt>true if and only if this
	 * list contains at least one element <tt>e such that <tt>(o==null ? e==null : o == e).
	 *
	 * @param o
	 ...
	 * contain the element. More formally, returns the lowest index <tt>i such that <tt>(o==null ? get(i)==null : o ==
	 ...
	 * contain the element. More formally, returns the highest index <tt>i such that <tt>(o==null ? get(i)==null : o ==
	 * get(i)), or -1 if there is no such index.
	 */
	@Override
	 ...
	 * element in the array immediately following the end of the collection is set to <tt>null. (This is useful in
	 * determining the length of the list <i>only if the caller knows that the list does not contain any null elements.)
	 *
	 * @param a
	 ...
	 * contain the element, it is unchanged. More formally, removes the element with the lowest index <tt>i such that
	 * <tt>(o==null ? get(i)==null : o == get(i)) (if such an element exists). Returns <tt>true if this list contained
	...
	/**
	 * Removes from this list all of the elements whose index is between <tt>fromIndex, inclusive, and toIndex,
	 * exclusive. Shifts any succeeding elements to the left (reduces their index). This call shortens the list by
	 * <tt>(toIndex - fromIndex) elements. (If <tt>toIndex==fromIndex, this operation has no effect.)
	 *
	 * @param fromIndex
	 ...
	 * @throws IndexOutOfBoundsException
	 *             if fromIndex or toIndex out of range (fromIndex < 0 || fromIndex >= size() || toIndex > size() ||
	 *             toIndex < fromIndex)
	 */
	@Override
