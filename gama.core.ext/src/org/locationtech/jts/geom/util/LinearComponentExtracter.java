...
/**
 * Extracts all the 1-dimensional ({@link LineString}) components from a {@link Geometry}. For polygonal geometries,
 * this will extract all the component {@link LinearRing}s. If desired, <code>LinearRing</code>s can be forced to be
 * returned as <code>LineString</code>s.
 *
 * @version 1.7
 */
public class LinearComponentExtracter implements GeometryComponentFilter {
	 ...
	 */
	public static Collection getLines(final Collection<Geometry> geoms, final Collection<Geometry> lines) {
	 ...
	 * @return the Collection of linear components (LineStrings or LinearRings)
	 */
	public static Collection getLines(final Geometry geom, final Collection<Geometry> lines) {
	...
	/**
	 * Extracts the linear components from a single geometry. If more than one geometry is to be processed, it is more
	 * efficient to create a single {@link LinearComponentExtracter} instance and pass it to multiple geometries.
	 *
	 * @param geom
	 *            the geometry from which to extract linear components
	 ...
	 * @return the list of linear components
	 */
	public static List getLines(final Geometry geom, final boolean forceToLineString) {
	...
	/**
	 * Extracts the linear components from a single {@link Geometry} and returns them as either a {@link LineString} or
	 ...
	 */
	public static Geometry getGeometry(final Geometry geom) {
	 ...
	 * @return a linear geometry
	 */
	public static Geometry getGeometry(final Geometry geom, final boolean forceToLineString) {
	 ...
	 */
	public LinearComponentExtracter(final Collection<Geometry> lines) {
		this.lines = lines;
	}
	 ...
	 * Constructs a LineExtracterFilter with a list in which to store LineStrings found.
	 */
	public LinearComponentExtracter(final Collection<Geometry> lines, final boolean isForcedToLineString) {
		this.lines = lines;
	...
	public void filter(final Geometry geom) {
		if (isForcedToLineString && geom instanceof LinearRing) {
			final LineString line = geom.getFactory().createLineString(((LinearRing) geom).getCoordinateSequence());
			lines.add(line);
			return;
		}
