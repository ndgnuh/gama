 ...
 *
 ********************************************************************************************************/
package gama.extensions.graph.layout;
...
public abstract class PrefuseStaticLayoutAbstract implements IStaticLayout {
	...
	/**
	 * Takes a gama.extensions.graph graph and applies a gama.extensions.graph layout, with a max time for execution; the layout is bounded
	...
	private void renderLayout(final Graph prefuseGraph, final Layout prefuseLayout, final Rectangle2D bounds,
		...
		while (itPrefuseNodes.hasNext()) {
			...
			if (pn instanceof IShape) {
				final IShape gamaNode = (IShape) pn;
				final GamaPoint newloc = new GamaPoint(prefuseNode.getX(), prefuseNode.getY());
	...
	/**
	 * Takes a gama.extensions.graph graph and applies a gama.extensions.graph layout, with a max time for execution. Layout will use the space
	...
	/**
	 * The actual creation of the gama.extensions.graph layout to be used by the layout process.
	 *
	 * @param timeout
	 * @param options
	 * @return
