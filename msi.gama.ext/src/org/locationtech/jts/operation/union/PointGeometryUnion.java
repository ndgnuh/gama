...
/**
 * Computes the union of a {@link Puntal} geometry with another arbitrary {@link Geometry}. Does not copy any component
 * geometries.
 *
 * @author mbdavis
 *
...
public class PointGeometryUnion {
	...
	public Geometry union() {
		...
		final Set<Coordinate> exteriorCoords = new TreeSet<>();

		for (int i = 0; i < pointGeom.getNumGeometries(); i++) {
			final Point point = (Point) pointGeom.getGeometryN(i);
		...
		final Coordinate[] coords = CoordinateArrays.toCoordinateArray(exteriorCoords);
		if (coords.length == 1) {
