...
/**
 * Polygonizes a set of {@link Geometry}s which contain linework that represents the edges of a planar graph. All types
 * of Geometry are accepted as input; the constituent linework is extracted as the edges to be polygonized. The
 * processed edges must be correctly noded; that is, they must only meet at their endpoints. Polygonization will accept
 * incorrectly noded input but will not form polygons from non-noded edges, and reports them as errors.
 * <p>
 ...
 * <li><b>Invalid Ring Lines</b> - edges which form rings which are invalid (e.g. the component lines contain a
 * self-intersection)
 * </ul>
 * Polygonization supports extracting only polygons which form a valid polygonal geometry. The set of extracted polygons
 * is guaranteed to be edge-disjoint. This is useful for situations where it is known that the input lines form a valid
 * polygonal geometry.
 *
 * @version 1.7
 */
public class Polygonizer {
	...
	private static class LineStringAdder implements GeometryComponentFilter {
		...
		LineStringAdder(final Polygonizer p) {
			this.p = p;
		}
	...
	/**
	 * Creates a polygonizer with the same {@link GeometryFactory} as the input {@link Geometry}s. The output mask is
	 * {@link #ALL_POLYS}.
	 */
	public Polygonizer() {
		this(false);
	...
	/**
	 * Creates a polygonizer and allow specifying if only polygons which form a valid polygonal geometry are to be
	 * extracted.
	 *
	 * @param extractOnlyPolygonal
	 *            true if only polygons which form a valid polygonal geometry are to be extracted
	 */
	public Polygonizer(final boolean extractOnlyPolygonal) {
		this.extractOnlyPolygonal = extractOnlyPolygonal;
	}

	/**
	 * Adds a collection of geometries to the edges to be polygonized. May be called multiple times. Any dimension of
	 * Geometry may be added; the constituent linework will be extracted and used.
	 *
	 * @param geomList
	...
	/**
	 * Add a {@link Geometry} to the edges to be polygonized. May be called multiple times. Any dimension of Geometry
	 * may be added; the constituent linework will be extracted and used
	 *
	 * @param g
	...
	/**
	 * Gets a geometry representing the polygons formed by the polygonization. If a valid polygonal geometry was
	 * extracted the result is a {@link Polygonal} geometry.
	 *
	...
	public Geometry getGeometry() {
		...
		polygonize();
		if (extractOnlyPolygonal) { return geomFactory.buildGeometry(polyList); }
	...
	private void polygonize() {
		...
		final List<EdgeRing> edgeRingList = graph.getEdgeRings();
	...
	private void findValidRings(final List<EdgeRing> edgeRingList, final List<EdgeRing> validEdgeRingList,
			final List<LineString> invalidRingList) {
		for (final Object element : edgeRingList) {
			final EdgeRing er = (EdgeRing) element;
			if (er.isValid()) {
	...
	private static void assignHolesToShells(final List holeList, final List shellList) {
		for (final Iterator i = holeList.iterator(); i.hasNext();) {
			...
			/*
			 * if ( ! holeER.hasShell()) { System.out.println("DEBUG: Outer hole: " + holeER); }
			 */
		}
	}
	...
	private static void findDisjointShells(final List shellList) {
	...
	/**
	 * For each outer hole finds and includes a single outer shell. This seeds the traversal algorithm for finding only
	 * polygonal shells.
	 *
	 * @param shellList
	 *            the list of shell EdgeRings
	 */
	private static void findOuterShells(final List shellList) {

		for (final Iterator i = shellList.iterator(); i.hasNext();) {
			final EdgeRing er = (EdgeRing) i.next();
			final EdgeRing outerHoleER = er.getOuterHole();
	...
	private static List extractPolygons(final List shellList, final boolean includeAll) {
