...
/**
 * Base class for STRtree and SIRtree. STR-packed R-trees are described in: P. Rigaux, Michel Scholl and Agnes Voisard.
 * <i>Spatial Databases With Application To GIS.</i> Morgan Kaufmann, San Francisco, 2002.
 * <p>
 * This implementation is based on {@link Boundable}s rather than {@link AbstractNode}s, because the STR algorithm
 * operates on both nodes and data, both of which are treated as Boundables.
 * <p>
...
public abstract class AbstractSTRtree implements Serializable {
	...
	/**
	 * A test for intersection between two bounds, necessary because subclasses of AbstractSTRtree have different
	 * implementations of bounds.
	 */
	protected interface IntersectsOp {
		/**
		 * For STRtrees, the bounds will be Envelopes; for SIRtrees, Intervals; for other subclasses of AbstractSTRtree,
		 * some other class.
		 * 
		 * @param aBounds
	...
	/**
	 * Creates parent nodes, grandparent nodes, and so forth up to the root node, for the data that has been inserted
	 * into the tree. Can only be called once, and thus can be called only after all of the data has been inserted into
	 * the tree.
	 */
	...
	protected List<Boundable> createParentBoundables(final List<Boundable> childBoundables, final int newLevel) {
		...
		final ArrayList<Boundable> parentBoundables = new ArrayList<>();
		...
		for (final Boundable boundable : sortedChildBoundables) {
			final Boundable childBoundable = boundable;
	...
	protected AbstractNode lastNode(final List<Boundable> nodes) {
	 ...
	 * @param level
	 *            the level of the Boundables, or -1 if the boundables are item boundables (that is, below level 0)
	 * @return the root, which may be a ParentNode or a LeafNode
	 */
	private AbstractNode createHigherLevels(final List<Boundable> boundablesOfALevel, final int level) {
		Assert.isTrue(!boundablesOfALevel.isEmpty());
		final List<Boundable> parentBoundables = createParentBoundables(boundablesOfALevel, level + 1);
		if (parentBoundables.size() == 1) { return (AbstractNode) parentBoundables.get(0); }
	...
	/**
	 * Tests whether the index contains any items. This method does not build the index, so items can still be inserted
	 ...
	 */
	protected List<Object> query(final Object searchBounds) {
		build();
	...
	/**
	 * @return a test for intersection between two bounds, necessary because subclasses of AbstractSTRtree have
	 *         different implementations of bounds.
	 * @see IntersectsOp
	 */
	protected abstract IntersectsOp getIntersectsOp();

	private void queryInternal(final Object searchBounds, final AbstractNode node, final List<Object> matches) {
	...
	private void queryInternal(final Object searchBounds, final AbstractNode node, final ItemVisitor visitor) {
		final List<?> childBoundables = node.getChildBoundables();
		for (int i = 0; i < childBoundables.size(); i++) {
			final Boundable childBoundable = (Boundable) childBoundables.get(i);
			if (!getIntersectsOp().intersects(childBoundable.getBounds(), searchBounds)) {
	 ...
	 * <p>
	 * The returned {@link List}s contain either {@link Object} items, or Lists which correspond to subtrees of the tree
	...
	public List<Object> itemsTree() {
		...
		final List<Object> valuesTree = itemsTree(root);
		if (valuesTree == null) { return new ArrayList<>(); }
		return valuesTree;
	}

	private List<Object> itemsTree(final AbstractNode node) {
		final List<Object> valuesTreeForNode = new ArrayList<>();
		for (final Iterator<?> i = node.getChildBoundables().iterator(); i.hasNext();) {
			final Boundable childBoundable = (Boundable) i.next();
			if (childBoundable instanceof AbstractNode) {
				final List<Object> valuesTreeForChild = itemsTree((AbstractNode) childBoundable);
	...
	protected List<Boundable> boundablesAtLevel(final int level) {
		final ArrayList<Boundable> boundables = new ArrayList<>();
	 ...
	 *            -1 to get items
	 */
	private void boundablesAtLevel(final int level, final AbstractNode top, final Collection<Boundable> boundables) {
