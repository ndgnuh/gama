...
/**
 * Tests whether a <code>Geometry</code> is simple. In general, the SFS specification of simplicity follows the rule:
 * <ul>
 ...
 * <ul>
 * <li><b>Polygonal</b> geometries are simple by definition, so <code>isSimple</code> trivially returns true. (Note:
 * this means that <tt>isSimple</tt> cannot be used to test for (invalid) self-intersections in <tt>Polygon</tt>s. In
 * order to check if a <tt>Polygonal</tt> geometry has self-intersections, use {@link Geometry#isValid()}).
 * <li><b>Linear</b> geometries are simple iff they do <i>not</i> self-intersect at interior points (i.e. points other
 * than boundary points). This is equivalent to saying that no two linear components satisfy the SFS
 * {@link Geometry#touches(Geometry)} predicate.
 * <li><b>Zero-dimensional (point)</b> geometries are simple if and only if they have no repeated points.
 * <li><b>Empty</b> geometries are <i>always</i> simple, by definition
 * </ul>
 * For {@link Lineal} geometries the evaluation of simplicity can be customized by supplying a {@link BoundaryNodeRule}
 * to define how boundary points are determined. The default is the SFS-standard
 * {@link BoundaryNodeRule#MOD2_BOUNDARY_RULE}. Note that under the <tt>Mod-2</tt> rule, closed <tt>LineString</tt>s
 * (rings) will never satisfy the <tt>touches</tt> predicate at their endpoints, since these are interior points, not
 * boundary points. If it is required to test whether a set of <code>LineString</code>s touch only at their endpoints,
 * use <code>IsSimpleOp</code> with {@link BoundaryNodeRule#ENDPOINT_BOUNDARY_RULE}. For example, this can be used to
...
public class IsSimpleOp {
	...
	/**
	 * Gets a coordinate for the location where the geometry fails to be simple. (i.e. where it has a non-boundary
	 * self-intersection). {@link #isSimple} must be called before this method is called.
	 *
	...
	private boolean isSimpleMultiPoint(final MultiPoint mp) {
		...
		final Set<Coordinate> points = new TreeSet<>();
		for (int i = 0; i < mp.getNumGeometries(); i++) {
			final Point pt = (Point) mp.getGeometryN(i);
			final Coordinate p = pt.getCoordinate();
	...
	/**
	 * Computes simplicity for polygonal geometries. Polygonal geometries are simple if and only if all of their
	 * component rings are simple.
	 * 
	 * @param geom
	...
	private boolean isSimpleGeometryCollection(final Geometry geom) {
		for (int i = 0; i < geom.getNumGeometries(); i++) {
			final Geometry comp = geom.getGeometryN(i);
	...
	/**
	 * For all edges, check if there are any intersections which are NOT at an endpoint. The Geometry is not simple if
	 * there are intersections not at endpoints.
	 */
	...
	/**
	 * Tests that no edge intersection is the endpoint of a closed line. This ensures that closed lines are not touched
	 * at their endpoint, which is an interior point according to the Mod-2 rule To check this we compute the degree of
	 * each endpoint. The degree of endpoints of closed lines must be exactly 2.
	 */
