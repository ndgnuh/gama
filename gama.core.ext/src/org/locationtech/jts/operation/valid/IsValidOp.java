...
/**
 * Implements the algorithms required to compute the <code>isValid()</code> method for {@link Geometry}s. See the
 * documentation for the various geometry types for a specification of validity.
 *
 * @version 1.7
 */
public class IsValidOp {
	...
	/**
	 * Checks whether a coordinate is valid for processing. Coordinates are valid iff their x and y ordinates are in the
	...
	public static Coordinate findPtNotNode(final Coordinate[] testCoords, final LinearRing searchRing,
		...
		for (final Coordinate pt : testCoords) {
	...
	/**
	 * Sets whether polygons using <b>Self-Touching Rings</b> to form holes are reported as valid. If this flag is set,
	 * the following Self-Touching conditions are treated as being valid:
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * The default (following the OGC SFS standard) is that this condition is <b>not</b> valid (<code>false</code>).
	 * <p>
	 * This does not affect whether Self-Touching Rings disconnecting the polygon interior are considered valid (these
	 * are considered to be <b>invalid</b> under the SFS, and many other spatial models as well). This includes
	 * "bow-tie" shells, which self-touch at a single point causing the interior to be disconnected, and "C-shaped"
	 * holes which self-touch at a single point causing an island to be formed.
	 *
	 * @param isValid
	...
	/**
	 * Computes the validity of the geometry, and if not valid returns the validation error for the geometry, or null if
	...
	private void checkValid(final MultiPolygon g) {
		for (int i = 0; i < g.getNumGeometries(); i++) {
			final Polygon p = (Polygon) g.getGeometryN(i);
	...
	private void checkInvalidCoordinates(final Coordinate[] coords) {
		for (final Coordinate coord : coords) {
			if (!isValid(coord)) {
				validErr = new TopologyValidationError(TopologyValidationError.INVALID_COORDINATE, coord);
				return;
			}
		}
	}
	...
	/**
	 * Check that there is no ring which self-intersects (except of course at its endpoints). This is required by OGC
	 * topology rules (but not by other models such as ESRI SDE, which allow inverted shells and exverted holes).
	 *
	 * @param graph
	...
	/**
	 * Check that a ring does not self-intersect, except at its endpoints. Algorithm is to count the number of times
	 * each node along edge occurs. If any occur more than once, that must be a self-intersection.
	 */
	private void checkNoSelfIntersectingRing(final EdgeIntersectionList eiList) {
		final Set<Coordinate> nodeSet = new TreeSet<>();
	...
	/**
	 * Tests that each hole is inside the polygon shell. This routine assumes that the holes have previously been tested
	 * to ensure that all vertices lie on the shell or on the same side of it (i.e. that the hole rings do not cross the
	 * shell ring). In other words, this test is only correct if the ConsistentArea test is passed first. Given this, a
	 * simple point-in-polygon test of a single point in the hole can be used, provided the point is chosen such that it
	 * does not lie on the shell.
	 *
	 * @param p
	...
	private void checkHolesInShell(final Polygon p, final GeometryGraph graph) {
		...
		final boolean isShellEmpty = shell.isEmpty();
		...
		final PointOnGeometryLocator pir = new IndexedPointInAreaLocator(shell);
		...
		for (int i = 0; i < p.getNumInteriorRing(); i++) {
			...
			/**
			 * If no non-node hole vertex can be found, the hole must split the polygon into disconnected interiors.
			 * This will be caught by a subsequent check.
			 */
			if (holePt == null) { return; }

			final boolean outside = isShellEmpty || Location.EXTERIOR == pir.locate(holePt);
			if (outside) {
	 ...
	 * this, holes have previously been tested to ensure that:
	 * <ul>
	 * <li>they do not partially overlap (checked by <code>checkRelateConsistency</code>)
	...
	private void checkHolesNotNested(final Polygon p, final GeometryGraph graph) {
		...
		if (!isNonNested) {
			validErr = new TopologyValidationError(TopologyValidationError.NESTED_HOLES, nestedTester.getNestedPoint());
		}
	}
	 ...
	 * </ul>
	 * This routine relies on the fact that while polygon shells may touch at one or more vertices, they cannot touch at
	 * ALL vertices.
	 */
	private void checkShellsNotNested(final MultiPolygon mp, final GeometryGraph graph) {
		for (int i = 0; i < mp.getNumGeometries(); i++) {
			...
			for (int j = 0; j < mp.getNumGeometries(); j++) {
				...
				final Polygon p2 = (Polygon) mp.getGeometryN(j);
	...
	/**
	 * Check if a shell is incorrectly nested within a polygon. This is the case if the shell is inside the polygon
	 * shell, but not inside a polygon hole. (If the shell is inside a polygon hole, the nesting is valid.)
	 * <p>
	 * The algorithm used relies on the fact that the rings must be properly contained. E.g. they cannot partially
	 * overlap (this has been previously checked by <code>checkRelateConsistency</code> )
	 */
	private void checkShellNotNested(final LinearRing shell, final Polygon p, final GeometryGraph graph) {
		...
		final boolean insidePolyShell = PointLocation.isInRing(shellPt, polyPts);
		...
		/**
		 * Check if the shell is inside one of the holes. This is the case if one of the calls to checkShellInsideHole
		 * returns a null coordinate. Otherwise, the shell is not properly contained in a hole, which is an error.
		 */
	...
	/**
	 * This routine checks to see if a shell is properly contained in a hole. It assumes that the edges of the shell and
	 * hole do not properly intersect.
	 *
	 * @return <code>null</code> if the shell is properly contained, or a Coordinate which is not inside the hole if it
	 *         is not
	 *
	 */
	private Coordinate checkShellInsideHole(final LinearRing shell, final LinearRing hole, final GeometryGraph graph) {
		final Coordinate[] shellPts = shell.getCoordinates();
		...
		if (shellPt != null) {
			final boolean insideHole = PointLocation.isInRing(shellPt, holePts);
			if (!insideHole) { return shellPt; }
		}
		final Coordinate holePt = findPtNotNode(holePts, shell, graph);
		// if point is on hole but not shell, check that the hole is outside the shell
		if (holePt != null) {
			final boolean insideShell = PointLocation.isInRing(holePt, shellPts);
	...
	private void checkConnectedInteriors(final GeometryGraph graph) {
		...
		if (!cit.isInteriorsConnected()) {
			validErr = new TopologyValidationError(TopologyValidationError.DISCONNECTED_INTERIOR, cit.getCoordinate());
		}
	}
