...
abstract public class ShapeElement extends RenderableElement {
	 ...
	 * new font glyph sets at different sizes.
	 */
	...
	/*
	 * protected void setStrokeWidthScalar(float strokeWidthScalar) { this.strokeWidthScalar = strokeWidthScalar; }
	 */
	...
	protected void renderShape(final Graphics2D g, final Shape shape) throws SVGException {
			...
			if (styleAttrib.getStringValue().equals("none")) {
				...
				if (fillPaint == null) {
					...
					if (uri != null) {
						...
						final AffineTransform xform = g.getTransform();

						final SVGElement ele = diagram.getUniverse().getElement(uri);
		...
		if (getStyle(styleAttrib.setName("stroke-linecap"))) {
			final String val = styleAttrib.getStringValue();
			if (val.equals("round")) {
		...
		if (fillPaint != null && fillOpacity != 0f) {
			if (fillOpacity <= 0) {
				...
				final Composite cachedComposite = g.getComposite();
		...
		if (strokePaint != null && strokeOpacity != 0f) {
			...
			final Shape strokeShape = stroke.createStrokedShape(shape);
	...
	protected Rectangle2D includeStrokeInBounds(final Rectangle2D rect) throws SVGException {
		final StyleAttribute styleAttrib = new StyleAttribute();
