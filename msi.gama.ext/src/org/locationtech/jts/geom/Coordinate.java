 ...
 * <p>
 * It is distinct from {@link Point}, which is a subclass of {@link Geometry}. Unlike objects of type {@link Point}
 * (which contain additional information such as an envelope, a precision model, and spatial reference system
 * information), a <code>Coordinate</code> only contains ordinate values and accessor methods.
 * </p>
 * <p>
 * <code>Coordinate</code>s are two-dimensional points, with an additional Z-ordinate. If an Z-ordinate value is not
 * specified or not defined, constructed coordinates have a Z-ordinate of <code>NaN</code> (which is also the value of
 * <code>NULL_ORDINATE</code>). The standard comparison functions ignore the Z-ordinate. Apart from the basic accessor
 * functions, JTS supports only specific operations involving the Z-ordinate.
 * </p>
 * <p>
 * Implementations may optionally support Z-ordiante and M-measure values as appropriate for a CoordinateSeqeunce. Use
 * of {@link #getZ()} and {@link #getM()} accessors, or {@link #getOrdinate(int)} are recommended.
 * </p>
 *
 * @version 1.16
 */
public class Coordinate implements Comparable<Coordinate>, Cloneable, Serializable {
	...
	/**
	 * The value used to indicate a null or missing ordinate value. In particular, used for the value of ordinates for
	 * dimensions greater than the defined dimension of a coordinate.
	 */
	 ...
	 *
	 * <p>
	 * This constant assumes XYZM coordinate sequence definition, please check this assumption using
	 ...
	 *            the <code>Coordinate</code> to copy
	 */
	public void setCoordinate(final Coordinate other) {
		x = other.x;
		y = other.y;
		z = other.getZ();
	}
	 ...
	 * @return <code>true</code> if the x- and y-coordinates are equal; the z-coordinates do not have to be equal.
	 */
	public boolean equals2D(final Coordinate other) {
	 ...
	 * @return true if <code>other</code> is a <code>Coordinate</code> with the same values for X and Y.
	 */
	 ...
	 * @return true if <code>other</code> is a <code>Coordinate</code> with the same values for X, Y and Z.
	 */
	...
	/**
	 * Returns <code>true</code> if <code>other</code> has the same values for the x and y ordinates. Since Coordinates
	 * are 2.5D, this routine ignores the z value when making the comparison.
	 *
	 * @param other
	...
	/**
	 * Compares this {@link Coordinate} with the specified {@link Coordinate} for order. This method ignores the z value
	 * when making the comparison. Returns:
	 * <UL>
	...
	public Object clone() {
		try {
			final Coordinate coord = (Coordinate) super.clone();
	...
	public static class DimensionalComparator implements Comparator<Coordinate> {
		 ...
		 */
		public DimensionalComparator(final int dimensionsToTest) {
		...
		public int compare(final Coordinate c1, final Coordinate c2) {
			final int compX = compare(c1.x, c2.x);
			if (compX != 0) { return compX; }

			final int compY = compare(c1.y, c2.y);
			if (compY != 0) { return compY; }

			if (dimensionsToTest <= 2) { return 0; }

			final int compZ = compare(c1.getZ(), c2.getZ());
			return compZ;
		}
	}
