/*******************************************************************************************************
 *
 * gama.util.graph.GamaGraph.java, in plugin gama.core, is part of the source code of the GAMA modeling and simulation
 * platform (v. 1.8)
 *
...
import gama.metamodel.topology.graph.NBAStarPathfinder;
...
import gaml.operators.Graphs.EdgeToAdd;
import gaml.operators.Spatial.Creation;
...
public class GamaGraph<V, E> implements IGraph<V, E> {
	...
	protected void buildByEdge(final IScope scope, final IContainer edges) {
		if (edges != null) {
			for (final Object p : edges.iterable(scope)) {
				addEdge(p);
				final Object p2 = p instanceof gaml.operators.Graphs.GraphObjectToAdd
						? ((gaml.operators.Graphs.GraphObjectToAdd) p).getObject() : p;
	...
	public Object addEdge(final Object e) {
		...
		} else if (e instanceof gaml.operators.Graphs.GraphObjectToAdd) {
			addValue(graphScope, (gaml.operators.Graphs.GraphObjectToAdd) e);
			return ((gaml.operators.Graphs.GraphObjectToAdd) e).getObject();
		}
	...
	@Override
	public void addValue(final IScope scope, final gaml.operators.Graphs.GraphObjectToAdd value) {
		if (value instanceof gaml.operators.Graphs.EdgeToAdd) {
			final gaml.operators.Graphs.EdgeToAdd edge = (gaml.operators.Graphs.EdgeToAdd) value;
			if (edge.object == null) {
			...
			final gaml.operators.Graphs.NodeToAdd node = (gaml.operators.Graphs.NodeToAdd) value;
	...
	public void addValueAtIndex(final IScope scope, final Object idx,
			final gaml.operators.Graphs.GraphObjectToAdd value) {
	...
	public void addValues(final IScope scope, final IContainer values) {
		...
		for (final Object o : values.iterable(scope)) {
			if (o instanceof gaml.operators.Graphs.GraphObjectToAdd) {
				addValue(scope, (gaml.operators.Graphs.GraphObjectToAdd) o);
			}
		}
	...
	@Override
	public void setAllValues(final IScope scope, final gaml.operators.Graphs.GraphObjectToAdd value) {
	...
	public void removeValue(final IScope scope, final Object value) {
		...
		} else if (value instanceof gaml.operators.Graphs.NodeToAdd) {
			removeVertex(((gaml.operators.Graphs.NodeToAdd) value).object);
	 ...
	 *
	 * @see gama.common.interfaces.IContainer.Modifiable#removeIndexes(gama.runtime.scope.IScope,
	 *      gama.common.interfaces.IContainer)
	 */
	@Override
	...
	public boolean addVertex(final Object v) {
		if (v instanceof gaml.operators.Graphs.GraphObjectToAdd) {
			if (v instanceof IAgent) {
				if (!this.getVertices().isEmpty() && ((IAgent) v).getSpecies() != vertexSpecies) {
					vertexSpecies = null;
				}
			}
			addValue(graphScope, (gaml.operators.Graphs.GraphObjectToAdd) v);
			return ((gaml.operators.Graphs.GraphObjectToAdd) v).getObject() != null;
		}
	...
	protected IPath<V, E, IGraph<V, E>> pathFromEdges(final IScope scope, final V source, final V target,
			final IList<E> edges) {
		return PathFactory.create(this, source, target, edges);
	}
	...
	public IList<E> computeBestRouteBetween(final IScope scope, final V source, final V target) {
		...
		if (pathFindingAlgo == shortestPathAlgorithm.FloydWarshall) {
			...
			final GraphPath<V, E> path = optimizer.getShortestPath(scope, source, target);
			if (path == null)
				return GamaListFactory.create(getGamlType().getContentType());
			...
			if (saveComputedShortestPaths) {
				sp1 = shortestPathComputed.get(new Pair<>(source, target));
			}
			IList<E> spl1 = null;
			if (sp1 == null || sp1.isEmpty() || sp1.get(0).isEmpty()) {
				final NBAStarPathfinder<V, E> p1 = new NBAStarPathfinder<>(this, true);
				spl1 = p1.search(source, target);
				if (saveComputedShortestPaths) {
					saveShortestPaths(spl1, source, target);
				}
			} else {
				spl1 = GamaListFactory.create(scope, getGamlType().getContentType(), sp1.get(0));
			}
			return spl1;
		} else if (pathFindingAlgo == shortestPathAlgorithm.Dijkstra) {
			IList<IList<E>> sp2 = null;
			if (saveComputedShortestPaths) {
				sp2 = shortestPathComputed.get(new Pair<>(source, target));
			}
			IList<E> spl2 = null;
			...
			if (sp2 == null || sp2.isEmpty() || sp2.get(0).isEmpty()) {
				...
				try {
					...
					if (ph == null) {
						...
						if (re == null) {
							...
							spl2 = GamaListFactory.create(scope, getGamlType().getContentType(), re);
						}
					}
				...
				spl2 = GamaListFactory.create(scope, getGamlType().getContentType(), sp2.get(0));
			}
			return spl2;
			...
			if (saveComputedShortestPaths) {
				sp3 = shortestPathComputed.get(new Pair<>(source, target));
			}
			IList<E> spl3 = null;
			if (sp3 == null || sp3.isEmpty() || sp3.get(0).isEmpty()) {
				spl3 = GamaListFactory.create(getGamlType().getContentType());
				final gama.metamodel.topology.graph.AStar astarAlgo =
						new gama.metamodel.topology.graph.AStar(this, source, target);
				astarAlgo.compute();
				final List re = astarAlgo.getShortestPath();
				if (re == null) {
					spl3 = GamaListFactory.create(getGamlType().getContentType());
				} else {
					spl3 = GamaListFactory.create(scope, getGamlType().getContentType(), re);
				}
				if (saveComputedShortestPaths) {
				...
				spl3 = GamaListFactory.create(scope, getGamlType().getContentType(), sp3.get(0));
			}
			...
			if (sp1 == null || sp1.isEmpty() || sp1.get(0).isEmpty()) {
				...
				if (ph == null) {
					...
					if (re == null) {
						...
						spl1 = GamaListFactory.create(scope, getGamlType().getContentType(), re);
					}
				}
				if (saveComputedShortestPaths) {
					saveShortestPaths(spl1, source, target);
	...
	public List<E> getFromIndicesList(final IScope scope, final IList<GamaPair<V, V>> indices)
		...
		if (indices == null || indices.isEmpty(scope))
			return null;
	...
	public Boolean getConnected() {
		ConnectivityInspector c = new ConnectivityInspector(this);
	...
	/**
	 * @see gama.interfaces.IGamaContainer#any()
	 */
	@Override
		...
		if (optimizer != null) {
			for (int i = 0; i < vertices.size(); i++) {
				...
				for (int j = 0; j < vertices.size(); j++) {
					...
					final GraphPath<V, E> path = optimizer.getShortestPath(scope, v1, v2);
	...
	private Integer nextVertice(final IScope scope, final E edge, final V source, final IMap<V, Integer> indexVertices,
		...
		if (isDirected)
			return indexVertices.get(scope, (V) this.getEdgeTarget(edge));
	 ...
	 */
	@Override
	public gaml.operators.Graphs.GraphObjectToAdd buildValue(final IScope scope, final Object object) {
		if (object instanceof gaml.operators.Graphs.NodeToAdd)
			return new gaml.operators.Graphs.NodeToAdd(
					type.getKeyType().cast(scope, ((gaml.operators.Graphs.NodeToAdd) object).object, null, false),
					((gaml.operators.Graphs.NodeToAdd) object).weight);
		if (object instanceof gaml.operators.Graphs.EdgeToAdd)
			return new gaml.operators.Graphs.EdgeToAdd(
					type.getKeyType().cast(scope, ((gaml.operators.Graphs.EdgeToAdd) object).source, null, false),
					type.getKeyType().cast(scope, ((gaml.operators.Graphs.EdgeToAdd) object).target, null, false),
					type.getContentType().cast(scope, ((gaml.operators.Graphs.EdgeToAdd) object).object, null, false),
					((gaml.operators.Graphs.EdgeToAdd) object).weight);
		return new gaml.operators.Graphs.EdgeToAdd(null, null, type.getContentType().cast(scope, object, null, false),
				0.0);
	}
	 ...
	 * Method buildValues()
	 *
	 * @see gama.common.interfaces.IContainer.Modifiable#buildValues(gama.runtime.scope.IScope,
	 *      gama.common.interfaces.IContainer, gaml.types.IContainerType)
	 */
	@Override
	public IContainer<?, gaml.operators.Graphs.GraphObjectToAdd> buildValues(final IScope scope,
			final IContainer objects) {
		try (final Collector.AsList list = Collector.newList()) {
			if (!(objects instanceof gaml.operators.Graphs.NodesToAdd)) {
				for (final Object o : objects.iterable(scope)) {
					list.add(buildValue(scope, o));
				}
			} else {
				for (final Object o : objects.iterable(scope)) {
					list.add(buildValue(scope, new gaml.operators.Graphs.NodeToAdd(o)));
				}
			}
			return list.items();
		}
