...
/**
 * Computes the convex hull of a {@link Geometry}. The convex hull is the smallest convex Geometry that contains all the
...
public class ConvexHull {
	...
	/**
	 * Returns a {@link Geometry} that represents the convex hull of the input geometry. The returned geometry contains
	 * the minimal number of points needed to represent the convex hull. In particular, no more than two consecutive
	 * points will be collinear.
	 *
	 * @return if the convex hull contains 3 or more points, a {@link Polygon}; 2 points, a {@link LineString}; 1 point,
	 *         a {@link Point}; 0 points, an empty {@link GeometryCollection}.
	 */
	public Geometry getConvexHull() {

		if (inputPts.length == 0) { return geomFactory.createGeometryCollection(); }
		if (inputPts.length == 1) { return geomFactory.createPoint(inputPts[0]); }
		if (inputPts.length == 2) { return geomFactory.createLineString(inputPts); }
		...
		final Stack<Coordinate> cHS = grahamScan(sortedPts);
	 ...
	 */
	protected Coordinate[] toCoordinateArray(final Stack<Coordinate> stack) {
		final Coordinate[] coordinates = new Coordinate[stack.size()];
		for (int i = 0; i < stack.size(); i++) {
			final Coordinate coordinate = stack.get(i);
	...
	/**
	 * Uses a heuristic to reduce the number of points scanned to compute the hull. The heuristic is to find a polygon
	 * guaranteed to be in (or on) the hull, and eliminate all points inside it. A quadrilateral defined by the extremal
	 * points in the four orthogonal directions can be used, but even more inclusive is to use an octilateral defined by
	 * the points in the 8 cardinal directions.
	 * <p>
	 * Note that even if the method used to determine the polygon vertices is not 100% robust, this does not affect the
	...
	private Coordinate[] reduce(final Coordinate[] inputPts) {
		...
		final TreeSet<Coordinate> reducedSet = new TreeSet<>();
		for (final Coordinate polyPt : polyPts) {
			reducedSet.add(polyPt);
		}
		/**
		 * Add all unique points not in the interior poly. CGAlgorithms.isPointInRing is not defined for points actually
		 * on the ring, but this doesn't matter since the points of the interior polygon are forced to be in the reduced
		 * set.
		 */
		for (final Coordinate inputPt : inputPts) {
	 ...
	 */
	private Stack<Coordinate> grahamScan(final Coordinate[] c) {
		Coordinate p;
		final Stack<Coordinate> ps = new Stack<>();
		ps.push(c[0]);
		ps.push(c[1]);
		ps.push(c[2]);
	 ...
	 */
	private boolean isBetween(final Coordinate c1, final Coordinate c2, final Coordinate c3) {
	...
	/*
	 * // MD - no longer used, but keep for reference purposes private Coordinate[] computeQuad(Coordinate[] inputPts) {
	 * BigQuad bigQuad = bigQuad(inputPts);
	 *
	 * // Build a linear ring defining a big poly. ArrayList bigPoly = new ArrayList(); bigPoly.add(bigQuad.westmost);
	 * if (! bigPoly.contains(bigQuad.northmost)) { bigPoly.add(bigQuad.northmost); } if (!
	 * bigPoly.contains(bigQuad.eastmost)) { bigPoly.add(bigQuad.eastmost); } if (! bigPoly.contains(bigQuad.southmost))
	 * { bigPoly.add(bigQuad.southmost); } // points must all lie in a line if (bigPoly.size() < 3) { return null; } //
	 * closing point bigPoly.add(bigQuad.westmost);
	 *
	 * Coordinate[] bigPolyArray = CoordinateArrays.toCoordinateArray(bigPoly);
	 *
	 * return bigPolyArray; }
	 *
	 * private BigQuad bigQuad(Coordinate[] pts) { BigQuad bigQuad = new BigQuad(); bigQuad.northmost = pts[0];
	 * bigQuad.southmost = pts[0]; bigQuad.westmost = pts[0]; bigQuad.eastmost = pts[0]; for (int i = 1; i < pts.length;
	 * i++) { if (pts[i].x < bigQuad.westmost.x) { bigQuad.westmost = pts[i]; } if (pts[i].x > bigQuad.eastmost.x) {
	 * bigQuad.eastmost = pts[i]; } if (pts[i].y < bigQuad.southmost.y) { bigQuad.southmost = pts[i]; } if (pts[i].y >
	 * bigQuad.northmost.y) { bigQuad.northmost = pts[i]; } } return bigQuad; }
	 *
	 * private static class BigQuad { public Coordinate northmost; public Coordinate southmost; public Coordinate
	 * westmost; public Coordinate eastmost; }
	 */

	/**
	 * @param vertices
	 *            the vertices of a linear ring, which may or may not be flattened (i.e. vertices collinear)
	 * @return a 2-vertex <code>LineString</code> if the vertices are collinear; otherwise, a <code>Polygon</code> with
	...
	private Coordinate[] cleanRing(final Coordinate[] original) {
		...
		final ArrayList<Coordinate> cleanedRing = new ArrayList<>();
	 ...
	 * @version 1.7
	 */
	private static class RadialComparator implements Comparator<Object> {
		...
		/**
		 * Given two points p and q compare them with respect to their radial ordering about point o. First checks
		 * radial ordering. If points are collinear, the comparison is based on their distance to the origin.
		 * <p>
		 * p < q iff
		 * <ul>
		 ...
		 */
		private static int polarCompare(final Coordinate o, final Coordinate p, final Coordinate q) {
			...
			/*
			 * // MD - non-robust int result = 0; double alph = Math.atan2(dxp, dyp); double beta = Math.atan2(dxq,
			 * dyq); if (alph < beta) { result = -1; } if (alph > beta) { result = 1; } if (result != 0) return result;
			 * //
			 */

			final int orient = Orientation.index(o, p, q);
