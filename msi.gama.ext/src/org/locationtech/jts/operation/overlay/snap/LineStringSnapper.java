...
/**
 * Snaps the vertices and segments of a {@link LineString} to a set of target snap vertices. A snap distance tolerance
 * is used to control where snapping is performed.
 * <p>
...
public class LineStringSnapper {
	...
	private final LineSegment seg = new LineSegment(); // for reuse during snapping
	 ...
	 *            the snap tolerance to use
	 */
	public LineStringSnapper(final Coordinate[] srcPts, final double snapTolerance) {
		this.srcPts = srcPts;
	...
	private static boolean isClosed(final Coordinate[] pts) {
	 ...
	 */
	private void snapVertices(final CoordinateList srcCoords, final Coordinate[] snapPts) {
		// try snapping vertices
		// if src is a ring then don't snap final vertex
		final int end = isClosed ? srcCoords.size() - 1 : srcCoords.size();
		for (int i = 0; i < end; i++) {
			final Coordinate srcPt = srcCoords.get(i);
			final Coordinate snapVert = findSnapForVertex(srcPt, snapPts);
			if (snapVert != null) {
	...
	private Coordinate findSnapForVertex(final Coordinate pt, final Coordinate[] snapPts) {
		for (final Coordinate snapPt : snapPts) {
	...
	/**
	 * Snap segments of the source to nearby snap vertices. Source segments are "cracked" at a snap vertex. A single
	 * input segment may be snapped several times to different snap vertices.
	 * <p>
	 * For each distinct snap vertex, at most one source segment is snapped to. This prevents "cracking" multiple
	 * segments at the same point, which would likely cause topology collapse when being used on polygonal linework.
	 * 
	 * @param srcCoords
	 ...
	 */
	private void snapSegments(final CoordinateList srcCoords, final Coordinate[] snapPts) {
		...
		for (int i = 0; i < distinctPtCount; i++) {
			final Coordinate snapPt = snapPts[i];
			final int index = findSegmentIndexToSnap(snapPt, srcCoords);
			/**
			 * If a segment to snap to was found, "crack" it at the snap pt. The new pt is inserted immediately into the
			 * src segment list, so that subsequent snapping will take place on the modified segments. Duplicate points
			 * are not added.
			 */
			if (index >= 0) {
	 ...
	 * <p>
	 * Only a single segment is selected for snapping. This prevents multiple segments snapping to the same snap vertex,
	 * which would almost certainly cause invalid geometry to be created. (The heuristic approach to snapping used here
	 * is really only appropriate when snap pts snap to a unique spot on the src geometry.)
	 * <p>
	 ...
	 */
	private int findSegmentIndexToSnap(final Coordinate snapPt, final CoordinateList srcCoords) {
