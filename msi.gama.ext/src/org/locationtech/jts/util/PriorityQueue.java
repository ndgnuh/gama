...
public class PriorityQueue {
	...
	private final ArrayList<Object> items; // The queue binary heap array
	 ...
	 *            the item to insert.
	 */
	public void add(final Comparable<Object> x) {
	...
	public Object poll() {
		...
		final Object minItem = items.get(1);
	...
	private void reorder(int hole) {
		int child;
		final Object tmp = items.get(hole);

		for (; hole * 2 <= size; hole = child) {
			child = hole * 2;
			if (child != size && ((Comparable<Object>) items.get(child + 1)).compareTo(items.get(child)) < 0) {
				child++;
			}
			if (((Comparable<Object>) items.get(child)).compareTo(tmp) < 0) {
