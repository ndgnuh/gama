/*********************************************************************************************
 *
 * 'PickWorkspaceDialog.java, in plugin gama.core.application, is part of the source code of the GAMA modeling and
...
public class PickWorkspaceDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		try {
			...
			String wsRoot = WorkspaceManager.getLastSetWorkspaceDirectory();
			...
			rememberWorkspaceButton.setSelection(WorkspaceManager.remembersWorkspace());

			final String lastUsed = WorkspaceManager.lastUsedWorkspaces();
	...
	protected void okPressed() {
		...
		final String ret = WorkspaceManager.checkWorkspaceDirectory(str, true, true, cloning);
		if (ret != null) {
		...
		WorkspaceManager.remembersWorkspace(rememberWorkspaceButton.getSelection());
		WorkspaceManager.lastSetWorkspaceDirectory(buf.toString());
		...
		WorkspaceManager.setSelectedWorkspaceRootLocation(str);

		/* And on our preferences as well */
		// scope.getGui().debug("Writing " + str + " in the preferences");
		if (cloning) {
			final String previousLocation = WorkspaceManager.getLastSetWorkspaceDirectory();
			...
			if (workspaceDirectory != null) {
				...
				try {
					...
					WorkspaceManager.setApplyPrefs(true);
		...
		WorkspaceManager.lastSetWorkspaceDirectory(str);
	...
	public static boolean checkAndCreateWorkspaceRoot(final String wsRoot) {
		try {
			...
			File dotFile = new File(wsRoot + File.separator + WorkspaceManager.WS_IDENTIFIER);
			if (!dotFile.exists()) {
				final boolean created = dotFile.createNewFile();
				if (!created) { return false; }
				dotFile = new File(wsRoot + File.separator + WorkspaceManager.getModelIdentifier());
