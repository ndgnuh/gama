...
/**
 * Models a triangle formed from {@link QuadEdge}s in a {@link QuadEdgeSubdivision} which forms a triangulation. The
 * class provides methods to access the topological and geometric properties of the triangle and its neighbours in the
 * triangulation. Triangle vertices are ordered in CCW orientation in the structure.
 * <p>
 * QuadEdgeTriangles support having an external data attribute attached to them. Alternatively, this class can be
 * subclassed and attributes can be defined in the subclass. Subclasses will need to define their own
...
public class QuadEdgeTriangle {
	/**
	 * Creates {@link QuadEdgeTriangle}s for all facets of a {@link QuadEdgeSubdivision} representing a triangulation.
	 * The <tt>data</tt> attributes of the {@link QuadEdge}s in the subdivision will be set to point to the triangle
	 * which contains that edge. This allows tracing the neighbour triangles of any given triangle.
	 *
	 * @param subdiv
	...
	public static boolean contains(final Vertex[] tri, final Coordinate pt) {
		final Coordinate[] ring = new Coordinate[] { tri[0].getCoordinate(), tri[1].getCoordinate(),
				tri[2].getCoordinate(), tri[0].getCoordinate() };
	...
	public static boolean contains(final QuadEdge[] tri, final Coordinate pt) {
		final Coordinate[] ring = new Coordinate[] { tri[0].orig().getCoordinate(), tri[1].orig().getCoordinate(),
				tri[2].orig().getCoordinate(), tri[0].orig().getCoordinate() };
		return PointLocation.isInRing(pt, ring);
	}

	public static Geometry toPolygon(final Vertex[] v) {
		final Coordinate[] ringPts = new Coordinate[] { v[0].getCoordinate(), v[1].getCoordinate(),
				v[2].getCoordinate(), v[0].getCoordinate() };
		final GeometryFactory fact = new GeometryFactory();
		final LinearRing ring = fact.createLinearRing(ringPts);
		final Polygon tri = fact.createPolygon(ring);
		return tri;
	}

	public static Geometry toPolygon(final QuadEdge[] e) {
		final Coordinate[] ringPts = new Coordinate[] { e[0].orig().getCoordinate(), e[1].orig().getCoordinate(),
	 ...
	 * @param e
	 *            a QuadEdge
	 * @return the index of the edge in this triangle or -1 if the edge is not an edge of this triangle
	 */
	...
	public Polygon getGeometry(final GeometryFactory fact) {
		final LinearRing ring = fact.createLinearRing(getCoordinates());
		final Polygon tri = fact.createPolygon(ring);
		return tri;
	}
	...
	public List getTrianglesAdjacentToVertex(final int vertexIndex) {
		// Assert: isVertex
		final List<QuadEdgeTriangle> adjTris = new ArrayList<QuadEdgeTriangle>();
	...
	/**
	 * Gets the neighbours of this triangle. If there is no neighbour triangle, the array element is <code>null</code>
	 *
	...
	private static class QuadEdgeTriangleBuilderVisitor implements TriangleVisitor {
		private final List<QuadEdgeTriangle> triangles = new ArrayList<QuadEdgeTriangle>();
